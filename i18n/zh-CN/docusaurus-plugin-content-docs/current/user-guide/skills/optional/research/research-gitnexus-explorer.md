...
| | |
|---|---|
| 源 | 可选 — 使用 `hermes skills install official/research/gitnexus-explorer` 安装 |
...
将任何代码库索引到知识图谱中，并提供一个交互式 Web UI 来探索符号、调用链、聚类和执行流。通过 Cloudflare 隧道进行远程访问。

## 使用场景

- 用户想要直观地探索代码库的架构
- 用户请求仓库的知识图谱/依赖关系图
- 用户想要与他人共享一个交互式的代码库探索器
...
Web UI 会在浏览器中渲染所有节点。大约少于 5,000 个文件的仓库工作良好。大型仓库（3万+ 节点）可能会运行缓慢或使浏览器标签页崩溃。CLI/MCP 工具适用于任何规模 — 只有 Web 可视化存在此限制。
...
Web UI 默认使用 `localhost:4747` 进行 API 调用。修补它以使用同源策略，以便它能通过隧道/代理工作：
...
在 `server: { }` 块内添加 `allowedHosts: true`（仅当运行开发模式而非生产构建时需要）：
```typescript
server: {
    allowedHosts: true,
    // ... 现有配置
...
添加 `--embeddings` 以支持语义搜索（更慢 — 需要数分钟而非数秒）。

索引文件保存在仓库内的 `.gitnexus/` 目录中（自动被 gitignore）。

### 4. 创建代理脚本

将以下内容写入一个文件（例如，`$GITNEXUS_DIR/proxy.mjs`）。它提供生产版 Web UI 的服务，并将 `/api/*` 请求代理到 GitNexus 后端 — 同源，没有 CORS 问题，无需 sudo，无需 nginx。
...
# 启动隧道（--config /dev/null 避免与现有的命名隧道配置冲突）
cloudflared tunnel --config /dev/null --url http://localhost:8888 --no-autoupdate --protocol http2
```

隧道 URL（例如 `https://random-words.trycloudflare.com`）将打印到标准错误。分享它 — 任何拥有链接的人都可以探索该图谱。
...
- **对于 cloudflared，`--config /dev/null` 是必需的**，如果用户现有的命名隧道配置位于 `~/.cloudflared/config.yml`。没有它，配置中的通配符入口规则会为所有快速隧道请求返回 404。

- **隧道必须使用生产构建。** Vite 开发服务器默认会阻止非 localhost 主机（`allowedHosts`）。生产构建 + Node 代理完全避免了这个问题。

- **Web UI 不会创建 `.claude/` 或 `CLAUDE.md`。** 这些是由 `npx gitnexus analyze` 创建的。使用 `--skip-agents-md` 来抑制 markdown 文件的生成，然后 `rm -rf .claude/` 移除其余部分。这些是 hermes-agent 用户不需要的 Claude Code 集成。

- **浏览器内存限制。** Web UI 会将整个图谱加载到浏览器内存中。拥有 5k+ 文件的仓库可能会运行缓慢。30k+ 文件可能会导致标签页崩溃。

- **嵌入是可选的。** `--embeddings` 启用语义搜索，但在大型仓库上需要数分钟。快速探索时可以跳过它；如果您想通过 AI 聊天面板进行自然语言查询，请添加它。

- **多个仓库。** `gitnexus serve` 为所有已索引的仓库提供服务。索引多个仓库，启动一次 serve，Web UI 允许您在它们之间切换。