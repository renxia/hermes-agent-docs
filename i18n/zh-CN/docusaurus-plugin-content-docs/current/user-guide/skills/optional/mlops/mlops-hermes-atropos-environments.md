---
title: "Hermes Atropos 环境 — 为 Atropos 训练构建、测试和调试 Hermes 智能体强化学习环境"
sidebar_label: "Hermes Atropos 环境"
description: "为 Atropos 训练构建、测试和调试 Hermes 智能体强化学习环境"
---
...
为 Atropos 训练构建、测试和调试 Hermes 智能体强化学习环境。涵盖 HermesAgentBaseEnv 接口、奖励函数、智能体循环集成、使用工具进行评估、wandb 日志记录以及三种 CLI 模式（serve/process/evaluate）。在 hermes-agent 仓库中创建、审查或修复强化学习环境时使用。
...
用于在 hermes-agent 仓库中构建与 Atropos 训练框架集成的强化学习（RL）环境的指南。
...
Hermes 环境之所以特殊，是因为它们运行的是**带有工具调用的多轮智能体循环**，而不仅仅是单轮补全。基础环境处理循环；您实现任务和评分。
...
**重要提示：** 在运行任何测试、评估或数据生成命令之前，请务必询问用户他们希望如何处理推理。**不要**假设使用 OpenRouter 或任何特定端点。请提供以下选项：
...
2. **自托管 VLLM 端点** — 询问其基础 URL（例如 `http://localhost:8000/v1`）和模型名称。设置 `--openai.server_type vllm`。
3. **其他 OpenAI 兼容 API** — 询问基础 URL、模型名称以及任何必需的 API 密钥。设置 `--openai.server_type openai` 和 `--openai.health_check false`。
4. **本地 Atropos 训练服务器** — 用于带有实时训练循环的 `serve` 模式。默认 `http://localhost:8000/v1`。

一旦用户告知您其设置，请在本次会话的所有 CLI 命令中使用这些值。示例提示：
...
**关键**：`result` 是一个 `AgentResult` 对象，**不是**字典。它具有以下属性：
...
`ctx`（ToolContext）为您提供对智能体沙箱的终端/文件访问权限，以便进行验证：
```python
# 在智能体的沙箱中运行测试
...
**必须使用带有工具调用的完整智能体循环**，而不是单轮 `chat_completion`。
hermes-agent 环境的全部意义就在于智能体式的评估：
...
**陷阱**：`compute_reward` 会向指标缓冲区追加内容。在评估期间，这会污染训练指标。请回滚在评估期间添加到缓冲区中的条目。

## 配置类

始终使用 Pydantic Field 描述符创建自定义配置子类。您可以调整的关键继承字段包括：`enabled_toolsets`、`max_agent_turns`、`agent_temperature`、`system_prompt`、`terminal_backend`、`group_size`、`steps_per_eval`、`total_steps`。

## config_init() — 默认配置

类方法，返回 `(YourEnvConfig, [APIServerConfig(...)])`。对于 OpenRouter/外部 API，请将 server_type 设置为 "openai"。从环境变量加载 API 密钥。

## 三种 CLI 模式

```bash
# SERVE — 完整训练循环（连接 Atropos API 服务器）
...
1. **AgentResult 包含 .messages，而非 .final_response** — 通过反向遍历 result.messages 查找最后一个包含内容的 assistant 消息来提取最终响应。

2. **evaluate() 必须使用 HermesAgentLoop，而非 chat_completion** — 单轮 chat_completion 不支持工具调用。hermes-agent 基准测试的核心在于使用工具的智能体评估。

3. **不要重复调用 _llm_judge** — 如果 compute_reward 已调用该函数，则应从缓冲区中提取分数，而非在 evaluate() 中另行调用 judge。

4. **评估会污染训练缓冲区** — compute_reward 会向指标缓冲区追加数据。评估期间需回滚缓冲区条目，以保持训练指标的纯净。

5. **OpenRouter 必须设置 health_check=false** — OpenRouter 没有 /health 端点。

6. **在 evaluate 模式下设置 data_dir_to_save_evals** — 否则结果不会被保存。

7. **default_toolsets 类变量与 enabled_toolsets 配置** — 类变量仅为提示；实际控制工具解析的是配置字段。

8. **消息中的工具调用解析** — 工具调用是形如 `{"function": {"name": ..., "arguments": ...}}` 的字典。始终检查 `isinstance(tc, dict)`。

9. **ToolContext.cleanup()** — 始终在 finally 块中调用以释放沙箱资源。

10. **外部 API 的 server_type 必须为 "openai"** — 否则 Atropos 会假定使用本地 VLLM 服务器。

11. **始终询问用户的推理设置** — 切勿硬编码或假定特定提供商/模型。参见上文“推理设置”部分。

## 奖励函数模式

### LLM 评判（适用于开放式任务）
使用 `self.server.chat_completion()` 配合评分提示。解析 JSON 响应以获取浮点分数。当评判调用失败时，始终包含启发式回退（关键词重叠）。
...
2. **询问用户的推理设置**（参见上文“推理设置”部分）
3. **处理模式**（1 个样本）：验证 JSONL 输出包含有效 token、掩码和分数
4. **评估模式**：验证完整智能体循环是否使用工具运行，指标是否正确记录
...
class MyEnv(HermesAgentBaseEnv):
    ...
    async def get_next_item(self): ... # 循环遍历训练样本
    def format_prompt(self, item): ... # 样本 → 用户消息字符串
    async def compute_reward(self, item, result, ctx): ...  # 评分推演
