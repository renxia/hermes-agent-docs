...
在 macOS 上通过 FindMy.app 追踪 Apple 设备和 AirTag。
...
在 macOS 上通过 FindMy.app 追踪 Apple 设备和 AirTag。由于 Apple 没有为 FindMy 提供命令行接口，此技能使用 AppleScript 打开应用，并通过屏幕截图读取设备位置。

## 先决条件

- 已登录 iCloud 并启用“查找”应用的 **macOS**
- 设备/AirTag 已在“查找”中注册
...
- 屏幕截图需要屏幕录制权限
- AppleScript UI 自动化可能在不同 macOS 版本中失效
