...
`xurl` 是 X 开发者平台为 X API 提供的官方 CLI 工具。它支持常见操作的快捷命令，以及对任意 v2 端点的原始 curl 风格访问。所有命令均将 JSON 输出到 stdout。
...
此技能取代了较早的 `xitter` 技能（该技能封装了一个第三方 Python CLI）。`xurl` 由 X 开发者平台团队维护，支持带自动刷新的 OAuth 2.0 PKCE，并覆盖更广泛的 API 功能。
...
- **切勿**在智能体会话中推荐或执行包含内联密钥的认证命令。
- **切勿**在智能体会话中使用 `--verbose` / `-v` —— 它可能暴露认证头/令牌。
- 要验证凭据是否存在，请仅使用：`xurl auth status`。

智能体命令中禁止的标志（它们接受内联密钥）：
`--bearer-token`、`--consumer-key`、`--consumer-secret`、`--access-token`、`--token-secret`、`--client-id`、`--client-secret`

应用凭据注册和凭据轮换必须由用户在智能体会话之外手动完成。注册凭据后，用户使用 `xurl auth oauth2` 进行认证 —— 同样在智能体会话之外。令牌以 YAML 格式持久化到 `~/.xurl`。每个应用拥有独立的令牌。OAuth 2.0 令牌会自动刷新。
...
如果 `xurl` 已安装但 `auth status` 显示无应用或无令牌，则用户需要手动完成认证 —— 请参阅下一节。

---

## 一次性用户设置（用户在智能体外运行这些命令）

这些步骤必须由用户直接执行，**而非**由智能体执行，因为它们涉及粘贴密钥。引导用户阅读此部分；不要为他们执行这些命令。
...
4. 在本地注册应用（用户运行此命令）：
   ...
   ```
5. 认证（指定 `--app` 以将令牌绑定到你的应用）：
   ```bash
   xurl auth oauth2 --app my-app
   ```
   （这会打开浏览器以进行 OAuth 2.0 PKCE 流程。）

   如果 X 在 OAuth 后查找 `/2/users/me` 时返回 `UsernameNotFound` 错误或 403，请显式传递你的用户名（xurl v1.1.0+）：
   ```bash
   xurl auth oauth2 --app my-app YOUR_USERNAME
   ```
   这会将令牌绑定到你的用户名，并跳过有问题的 `/2/users/me` 调用。
6. 将应用设为默认应用，以便所有命令都使用它：
   ```bash
...
完成此操作后，智能体即可使用以下任何命令，无需进一步设置。OAuth 2.0 令牌会自动刷新。

> **常见陷阱：** 如果在 `xurl auth oauth2` 中省略 `--app my-app`，OAuth 令牌将保存到内置的 `default` 应用配置文件中 —— 该配置文件没有 client-id 或 client-secret。即使 OAuth 流程看似成功，命令也会因认证错误而失败。如果遇到此问题，请重新运行 `xurl auth oauth2 --app my-app` 和 `xurl auth default my-app`。
...
快捷命令涵盖了常见操作。对于其他任何操作，请使用针对任意 X API v2 端点的原始 curl 风格模式：
...
| `--verbose` | `-v` | **禁止在智能体会话中使用** —— 会泄露认证头部 |
| `--trace` | `-t` | 添加 `X-B3-Flags: 1` 追踪头部 |
...
- 任何错误都会返回非零退出码。
- API 错误仍会以 JSON 格式打印到 stdout，因此你可以解析它们。
- 身份验证错误 → 让用户在智能体会话外重新运行 `xurl auth oauth2`。
...
2. **检查默认应用是否具有凭据。** 解析 `auth status` 输出。默认应用标记为 `▸`。如果默认应用显示 `oauth2: (none)`，但另一个应用具有有效的 oauth2 用户，请告诉用户运行 `xurl auth default <that-app>` 来修复它。这是最常见的设置错误 —— 用户添加了一个自定义名称的应用，但从未将其设置为默认应用，因此 xurl 会持续尝试使用空的 `default` 配置文件。
3. 如果完全没有身份验证，请停止并引导用户查看“一次性用户设置”部分 —— 切勿尝试自行注册应用或传递密钥。
4. 从一个低成本的读取操作开始（`xurl whoami`、`xurl user @handle`、`xurl search ... -n 3`）以确认可访问性。
5. 在任何写入操作（发帖、回复、点赞、转发、私信、关注、屏蔽、删除）之前，请确认目标帖子/用户以及用户的意图。
...
| 症状 | 原因 | 修复方法 |
| --- | --- | --- |
| OAuth 流程成功后出现身份验证错误 | 令牌保存到了 `default` 应用（无 client-id/secret），而不是你命名的应用 | `xurl auth oauth2 --app my-app`，然后 `xurl auth default my-app` |
| OAuth 期间出现 `unauthorized_client` | 在 X 仪表板中将应用类型设置为“原生应用” | 在用户身份验证设置中更改为“Web 应用、自动化应用或机器人” |
...
- **范围：** OAuth 2.0 令牌使用宽泛的范围。特定操作上的 403 通常表示令牌缺少某个范围 —— 请用户重新运行 `xurl auth oauth2`。
...
- **令牌存储：** `~/.xurl` 是 YAML 格式。切勿将此文件读取或发送到 LLM 上下文。
...
- Hermes 适配：根据 Hermes 技能约定重新格式化；安全护栏保持原样。