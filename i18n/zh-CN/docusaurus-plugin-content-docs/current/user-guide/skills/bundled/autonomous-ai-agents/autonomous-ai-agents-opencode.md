...
使用 [OpenCode](https://opencode.ai) 作为由 Hermes 终端/进程工具编排的自主编码工作器。OpenCode 是一个与提供商无关、开源的 AI 编码智能体，具有 TUI 和 CLI。
...
- 在 TUI 中提交时，可能需要按两次 Enter 键（一次用于完成文本，一次用于发送）。
...
1. 对于一次性自动化，优先使用 `opencode run` — 它更简单且不需要 pty。
2. 仅在需要迭代时才使用交互式后台模式。
