...
description: "通过两个零 MCP 控制运行中的 TouchDesigner 实例——创建算子、设置参数、连接线缆、执行 Python、构建实时视觉效果"
---

{/* 本页面由 website/scripts/generate-skill-docs.py 从技能的 SKILL.md 文件自动生成。请编辑源文件 SKILL.md，而非本页。 */}

# Touchdesigner Mcp

通过两个零 MCP 控制运行中的 TouchDesigner 实例——创建算子、设置参数、连接线缆、执行 Python、构建实时视觉效果。包含 36 个原生工具。
...
| 标签 | `TouchDesigner`, `MCP`, `两个零`, `创意编程`, `实时视觉`, `生成艺术`, `音频反应`, `VJ`, `装置艺术`, `GLSL` |
...
---
title: "TouchDesigner 集成 (twozero MCP)"
description: "如何设置和使用 twozero.tox 连接 TouchDesigner 与 Hermes 智能体"
slug: "touchdesigner-integration"
last_updated: "2026-04-01"
version: "1.0"
tags: ["touchdesigner", "mcp", "创意编码", "实时图形"]
---
...
1.  **切勿猜测参数名称。** 首先为操作符类型调用 `td_get_par_info`。您关于 TD 2025.32 的训练数据是错误的。
2.  **如果触发了 `tdAttributeError`，立即停止。** 在继续之前，对失败的节点调用 `td_get_operator_info`。
3.  **切勿在脚本回调中硬编码绝对路径。** 使用 `me.parent()` / `scriptOp.parent()`。
4.  **优先使用原生 MCP 工具而非 `td_execute_python`。** 使用 `td_create_operator`、`td_set_operator_pars`、`td_get_errors` 等。仅对于复杂的多步骤逻辑才回退到 `td_execute_python`。
5.  **在构建前调用 `td_get_hints`。** 它会返回与您正在使用的操作符类型相关的特定模式。
...
36 个原生工具。免费插件（无需付款/许可证 — 已于 2026 年 4 月确认）。
具有上下文感知能力（知道选中的 OP、当前网络）。
...
- **非商业版 TD** 的分辨率上限为 1280×1280。使用 `outputresolution = 'custom'` 并显式设置宽度/高度。
...
```
为您计划使用的每种类型调用 td_get_par_info 和 op_type。
为您正在构建的主题（例如“glsl”、“音频反应”、“反馈”）调用 td_get_hints。
调用 td_get_focus 以查看用户所在位置和选中内容。
调用 td_get_network 以查看已存在的网络。
```
...
**重要：将清理和创建拆分为单独的 MCP 调用。** 在一个 `td_execute_python` 脚本中销毁并重新创建同名节点会导致“Invalid OP object”错误。参见陷阱 #11b。
...
| `td_get_operators_info` | 在一个调用中检查多个节点 |
| `td_get_network` | 查看指定路径的网络结构 |
| `td_get_errors` | 递归查找错误/警告 |
| `td_get_par_info` | 获取操作符类型的参数名称（取代发现） |
| `td_get_hints` | 构建前获取模式/提示 |
| `td_get_focus` | 查看哪个网络打开，哪个节点被选中 |
...
| `td_click_screen_point` | 点击屏幕截图中的某个点 |
| `td_screen_point_to_global` | 将截图像素坐标转换为绝对屏幕坐标 |

上表涵盖了典型创意工作流中使用的 32 个工具。其余 4 个工具（`td_project_quit`、`td_test_session`、`td_dev_log`、`td_clear_dev_log`）是管理/开发模式实用程序 — 参见 `references/mcp-tools.md` 获取完整的 36 个工具参考及完整的参数模式。

## 关键实现规则

**GLSL 时间：** GLSL TOP 中没有 `uTDCurrentTime`。使用值页面：
```python
...
备选：使用 `rgba32float` 格式的常量 TOP（8 位会被限制在 0-1，使着色器冻结）。

**反馈 TOP：** 使用 `top` 参数引用，而不是直接输入连线。“Not enough sources” 在第一次烹饪后解决。“Cook dependency loop” 警告是预期的。
...
**扩展：** `ext0object` 格式在常量模式下是 `"op('./datName').module.ClassName(me)"`。使用 `td_write_dat` 编辑扩展代码后，调用 `td_reinit_extension`。

**脚本回调：** 始终通过 `me.parent()` / `scriptOp.parent()` 使用相对路径。

**清理节点：** 在迭代前始终执行 `list(root.children)` 并进行 `child.valid` 检查。
...
rec.par.videocodec = 'prores'  # Apple ProRes — 在 macOS 上无许可证限制
...
**`TOP.save()` 对于动画无用** — 每次都捕获相同的 GPU 纹理。始终使用 MovieFileOut。

### 录制前：检查清单

1.  通过 `td_get_perf` **验证 FPS > 0**。如果 FPS=0，录制将是空的。参见陷阱 #38-39。
2.  通过 `td_get_screenshot` **验证着色器输出不是黑色**。黑色输出 = 着色器错误或缺少输入。参见陷阱 #8、#40。
3.  **如果录制带有音频：** 先触发音频开始，然后将录制延迟 3 帧。参见陷阱 #19。
4.  **在开始录制前设置输出路径** — 在同一个脚本中同时设置两者可能会发生竞争条件。
...
1. **AudioSpectrum 必须保持时间切片开启**。关闭 = 处理整个音频文件 → 24000+ 个采样 → CHOP 转 TOP 溢出。
2. **手动设置输出长度**为 256，通过 `outputmenu='setmanually'` 和 `outlength=256`。默认输出 22050 个采样。
3. **不要使用 Lag CHOP 进行频谱平滑。** Lag CHOP 在时间切片模式下运行，并将 256 个采样扩展到 2400+ 个，将所有值平均到接近零（~1e-06）。着色器接收不到可用数据。这是测试中音频同步失败的首要原因。
4. **也不要使用 Filter CHOP** — 同样的时间切片扩展问题会影响频谱数据。
5. **平滑处理应在 GLSL 着色器中进行**（如果需要），通过时间插值与反馈纹理实现：`mix(prevValue, newValue, 0.3)`。这能提供帧级精确同步且零管线延迟。
6. **CHOP 转 TOP 数据格式 = 'r'**，布局 = '行裁剪'。频谱输出为 256x2（立体声）。在 y=0.25 处采样第一个声道。
7. **Math 增益 = 10**（不是 5）。原始频谱值在低频范围约为 0.19。增益为 10 时，着色器可获得约 5.0 的可用值。
...
// 每个频段采样多个点并取平均以提高稳定性：
// 注意：y=0.25 代表第一个声道（立体声纹理为 256x2，第一行中心为 0.25）
...
- `td_execute_python` 作为 TD 进程用户，对 TD Python 环境和文件系统具有无限制访问权限。
- `setup.sh` 从官方 404zero.com URL 下载 twozero.tox。如有疑虑，请验证下载文件。
- 此技能不向本地主机之外发送数据。所有 MCP 通信均为本地。

## 参考资料

| 文件 | 内容 |
|------|------|
| `references/pitfalls.md` | 真实会话中的宝贵经验教训 |
...
| `references/geometry-comp.md` | 几何体 COMP：实例化、POP vs SOP、变形 |
...
| `references/projection-mapping.md` | 多窗口输出、边角定位、网格变形、边缘混合 |
...
| `references/3d-scene.md` | 灯光布局、阴影、IBL/立方体贴图、多相机、PBR |
