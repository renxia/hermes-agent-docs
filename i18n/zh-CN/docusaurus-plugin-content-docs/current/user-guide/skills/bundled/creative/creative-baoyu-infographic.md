---
title: "宝玉信息图 — 信息图：21 种布局 x 21 种样式（信息图，可视化）"
sidebar_label: "宝玉信息图"
description: "信息图：21 种布局 x 21 种样式（信息图，可视化）"
---
...
改编自 [baoyu-infographic](https://github.com/JimLiu/baoyu-skills)，适用于 Hermes 智能体的工具生态系统。

两个维度：**布局**（信息结构）× **风格**（视觉美学）。可以自由组合任意布局与任意风格。

## 何时使用

当用户要求创建信息图、可视化摘要、信息图形，或使用“信息图”、“可视化”、“高密度信息大图”等术语时，触发此技能。用户提供内容（文本、文件路径、URL 或主题），并可选择性地指定布局、风格、宽高比或语言。

## 选项

| 选项 | 值 |
|--------|--------|
| 布局 | 21 种选项（参见布局库），默认：bento-grid |
| 风格 | 21 种选项（参见风格库），默认：craft-handmade |
...
| `hand-drawn-edu` | 马卡龙柔和色调、手绘抖动、简笔画人物 |
...
如果快捷方式有**提示备注**，则将其附加到生成的提示中（第 5 步），作为额外的风格说明。

| 用户关键词 | 布局 | 推荐风格 | 默认宽高比 | 提示备注 |
|--------------|--------|--------------------|----------------|--------------|
...
Slug：主题生成的 2-4 个单词的 kebab-case 格式。冲突时：追加 `-YYYYMMDD-HHMMSS`。
...
**问题 1 — 组合**：展示 3 个以上布局×风格组合及其理由。请用户选择一个。

**问题 2 — 宽高比**：询问宽高比偏好（横向/纵向/方形或自定义 W:H）。
...
1. **数据完整性至关重要** — 切勿总结、转述或更改原始统计数据。“73% 增长”必须保留为“73% 增长”，而不是“显著增长”。
2. **剥离机密信息** — 在任何输出文件中包含内容之前，务必扫描源内容中的 API 密钥、令牌或凭据。
3. **每个部分一条信息** — 每个信息图部分应传达一个清晰的概念。信息过载会降低可读性。
4. **风格一致性** — 必须将参考文件中的样式定义一致地应用于整个信息图。切勿混合使用不同风格。
5. **image_generate 宽高比** — 该工具仅支持 `横向`、`纵向` 和 `正方形`。像 `3:4` 这样的自定义宽高比应映射到最接近的选项（本例中为纵向）。