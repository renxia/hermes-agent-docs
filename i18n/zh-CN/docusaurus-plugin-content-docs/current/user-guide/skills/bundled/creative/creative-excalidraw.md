---
title: "Excalidraw — 手绘 Excalidraw JSON 图表（架构图、流程图、时序图）"
sidebar_label: "Excalidraw"
description: "手绘 Excalidraw JSON 图表（架构图、流程图、时序图）"
---
...
手绘 Excalidraw JSON 图表（架构图、流程图、时序图）。
...
:::info
以下是 Hermes 在触发此技能时加载的完整技能定义。当技能激活时，智能体看到的指令即为此内容。
:::

# Excalidraw 图表技能

通过编写标准 Excalidraw 元素 JSON 并保存为 `.excalidraw` 文件来创建图表。这些文件可拖放至 [excalidraw.com](https://excalidraw.com) 进行查看和编辑。无需账户、无需 API 密钥、无需渲染库——只需 JSON。

## 使用场景

为架构图、流程图、时序图、概念图等生成 `.excalidraw` 文件。文件可在 excalidraw.com 上打开，或上传以生成可共享链接。
...
4. **可选：上传**，通过 `terminal` 调用 `scripts/upload.py` 生成可共享链接
...
此操作将文件上传至 excalidraw.com（无需账户），并打印一个可共享的 URL。需要安装 `cryptography` pip 包（`pip install cryptography`）。
...
> **警告：** 切勿在形状上使用 `"label": { "text": "..." }`。这不是有效的 Excalidraw 属性，将被静默忽略，导致形状显示为空白。您必须使用下方的容器绑定方法。

形状需包含 `boundElements` 列出文本，而文本需包含 `containerId` 指回形状：
```json
...
- 文本的 `x`/`y`/`width`/`height` 为近似值 —— Excalidraw 在加载时会重新计算
...
```
- `x` 为左边缘。若要在位置 `cx` 居中：`x = cx - (text.length * fontSize * 0.5) / 2`
...
- 元素之间至少留出 20-30 像素的间距
- 优先使用较少但较大的元素，而非大量微小元素
