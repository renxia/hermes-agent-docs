...
:::info
以下是触发此技能时，Hermes 加载的完整技能定义。这就是技能激活时智能体看到的指令。
:::

# ASCII 艺术技能

多个工具满足不同的 ASCII 艺术需求。所有工具都是本地 CLI 程序或免费的 REST API —— 无需 API 密钥。

## 工具 1：文本横幅 (pyfiglet —— 本地)

将文本渲染为大型 ASCII 艺术横幅。内建 571 种字体。
...
- 短文本 (1-8 个字符) 与 `doom` 或 `block` 等精细字体配合最佳
- 长文本与 `small` 或 `mini` 等紧凑字体配合更好

## 工具 2：文本横幅 (asciified API —— 远程，无需安装)

免费的 REST API，可将文本转换为 ASCII 艺术。250+ 种 FIGlet 字体。直接返回纯文本 —— 无需解析。当未安装 pyfiglet 或需要快速替代方案时使用此工具。
...
- 任何带有 curl 的终端均可工作 —— 无需 Python 或 pip

## 工具 3：Cowsay (消息艺术)

经典工具，将文本包裹在与 ASCII 字符相连的对话气泡中。
...
在任意文本周围绘制装饰性 ASCII 艺术边框/框架。70+ 种内建设计。
...
类似 pyfiglet，但具有 ANSI 颜色效果和视觉滤镜。非常适合终端视觉效果。
...
**注意**：toilet 输出 ANSI 转义码以实现颜色效果 —— 在终端中有效，但可能无法在所有上下文（例如纯文本文件、某些聊天平台）中渲染。
...
### 来源 A：ascii.co.uk (预制艺术推荐)

按主题组织的大型经典 ASCII 艺术收藏。艺术作品位于 HTML 的 `<pre>` 标签内。用 curl 获取页面，然后用一小段 Python 代码提取艺术作品。
...
返回一个随机的 GitHub Octocat 和一句智慧格言。无需认证。
...
当上述工具没有所需内容时，直接使用以下 Unicode 字符生成 ASCII 艺术：
...
1. **文本作为横幅** → 如果已安装 pyfiglet 则使用 pyfiglet，否则通过 curl 调用 asciified API
2. **用有趣的字符艺术包裹消息** → cowsay
3. **添加装饰性边框/框架** → boxes（可与 pyfiglet/asciified 结合使用）
4. **特定事物的艺术**（猫、火箭、龙）→ 通过 curl 访问 ascii.co.uk 并解析
