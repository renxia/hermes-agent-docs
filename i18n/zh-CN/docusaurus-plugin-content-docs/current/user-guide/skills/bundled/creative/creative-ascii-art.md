---
title: "ASCII 艺术 — ASCII art: pyfiglet, cowsay, boxes, image-to-ascii"
...
一个免费的 REST API，可将文本转换为 ASCII 艺术。包含 250+ 种 FIGlet 字体。直接返回纯文本 — 无需解析。当未安装 pyfiglet 或需要快速替代方案时使用此工具。
...
- 字体名称区分大小写；请使用字体端点获取确切名称。
- 可在任何支持 curl 的终端中使用 — 无需 Python 或 pip。
...
为任何文本绘制装饰性 ASCII 艺术边框/框架。内置 70+ 种设计。
...
类似于 pyfiglet，但带有 ANSI 颜色效果和视觉滤镜。非常适合终端美化。
...
**注意**：toilet 输出包含 ANSI 转义码的颜色 — 在终端中有效，但可能无法在所有环境（例如纯文本文件、某些聊天平台）中正确渲染。

## 工具 6：图像转 ASCII 艺术

将图像（PNG, JPEG, GIF, WEBP）转换为 ASCII 艺术。

### 选项 A：ascii-image-converter（推荐，现代工具）
...
ascii-image-converter image.png -b               # 布拉耶盲文字符
ascii-image-converter image.png -n               # 负片/反转效果
...
从网络上搜索策划好的 ASCII 艺术。使用 `terminal` 配合 `curl`。

### 来源 A：ascii.co.uk（推荐用于预制艺术）

大量按主题组织的经典 ASCII 艺术收藏。艺术作品位于 HTML `<pre>` 标签内。用 curl 获取页面，然后用一小段 Python 代码提取艺术。
...
- 人物：`skull`, `robot`, `angel`, `wizard`, `pirate`, `ninja`, `alien`
...
### 来源 B：GitHub Octocat API（趣味彩蛋）

返回一个随机的 GitHub Octocat 和一句智慧引言。无需认证。
...
这些免费服务直接返回 ASCII 艺术 — 非常适合作为趣味附加内容。
...
```bash
curl -s "wttr.in/London"          # 包含 ASCII 图形的完整天气报告
...
1. **文本作为横幅** → 如果已安装 pyfiglet，否则使用 curl 调用 ascii 风格 API
2. **用有趣的字符艺术包裹消息** → cowsay
3. **添加装饰性边框/框架** → boxes（可与 pyfiglet/ascii 风格结合使用）
