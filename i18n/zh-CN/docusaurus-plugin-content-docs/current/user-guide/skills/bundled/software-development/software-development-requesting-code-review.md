---
title: "请求代码审查 — 预提交审查：安全扫描、质量关卡、自动修复"
sidebar_label: "请求代码审查"
description: "预提交审查：安全扫描、质量关卡、自动修复"
---
...
代码落地前的自动化验证流水线。静态扫描、基于基线的质量关卡、独立的审查子智能体，以及自动修复循环。
...
- 在 git 仓库中完成对 2 个以上文件的编辑后
- 在子智能体驱动开发中每次任务完成后（两阶段审查）

**跳过情况：** 仅文档变更、纯配置调整，或用户明确说“跳过验证”。

**此技能 vs github-code-review：** 此技能在提交前验证**你的**变更。  
`github-code-review` 则用于在 GitHub 上以内联评论方式审查**他人**的拉取请求。
...
如果 `git diff --cached` 为空但 `git diff` 显示有变更，请告知用户先执行 `git add <files>`。如果仍为空，运行 `git status` —— 无需验证。
...
检测项目语言并运行相应工具。在变更前捕获失败数量作为 **baseline_failures**（暂存变更、运行、恢复）。只有你的变更引入的**新失败**才会阻止提交。
...
**基线对比：** 如果基线是干净的，而你的变更引入了失败，则属于回归。如果基线已有失败，则只统计**新增**的失败。
...
```python
delegate_task(
    goal="""你是一名独立的代码审查者。你完全不了解这些变更是如何实现的。请审查 git 差异并**仅返回有效的 JSON**。
...
启动第三个智能体上下文 —— 既不是你（实现者），也不是审查者。  
它仅修复报告的问题：

```python
delegate_task(
    goal="""你是一名代码修复智能体。仅修复下面列出的具体问题。  
...
**子智能体驱动开发：** 每次任务后均运行此技能作为质量关卡。  
两阶段审查（规范合规性 + 代码质量）使用此流水线。

**测试驱动开发：** 此流水线验证是否遵循了 TDD 原则 ——  
测试存在、测试通过、无回归。
