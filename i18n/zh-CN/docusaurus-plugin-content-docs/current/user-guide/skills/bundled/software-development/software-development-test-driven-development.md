...
先写测试。观察其失败。编写最少的代码使其通过。

**核心原则：** 如果没有看到测试失败，就不知道它是否测试了正确的内容。
...
**要求：**
- 每个测试一个行为
- 名称清晰且具有描述性（名称中有“and”？拆分它）
- 真实代码，而不是 mock（除非真正不可避免）
- 名称描述行为，而不是实现
...
**“测试之后也能实现相同的目标 — 这是精神而不是仪式”**

不。测试之后回答“这是做什么的？”测试优先回答“这应该做什么？”

测试之后会受到你的实现的影响。你测试的是你构建的内容，而不是所需的内容。测试优先迫使你在实现之前发现边缘情况。
...
| “我稍后会测试” | 测试立即通过证明不了任何事情。 |
| “测试之后也能实现相同的目标” | 测试之后 = “这是做什么的？”测试优先 = “这应该做什么？” |
| “已经手动测试过了” | 临时 ≠ 系统性。没有记录，无法重新运行。 |
| “删除 X 小时是浪费” | 沉没成本谬误。保留未经验证的代码是技术债务。 |
| “保留作为参考，先写测试” | 你会适配它。这就是测试之后。删除就是删除。 |
| “需要先探索” | 没问题。丢弃探索，从 TDD 开始。 |
