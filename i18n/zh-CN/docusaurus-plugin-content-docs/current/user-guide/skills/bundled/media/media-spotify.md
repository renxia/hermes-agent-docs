...
用户说类似“播放 X”、“暂停”、“跳过”、“将 X 加入队列”、“正在播放什么”、“搜索 X”、“添加到我的 X 播放列表”、“创建播放列表”、“保存到我的音乐库”等。
...
- `spotify_library` — 使用 `kind: "tracks"|"albums"` 进行列出/保存/移除

播放变更操作需要 Spotify Premium；搜索/音乐库/播放列表操作可在免费版上使用。

## 规范模式（最小化工具调用）

### “播放 &lt;艺术家/曲目/专辑>”
一次搜索，然后通过 URI 播放。除非用户要求选择，否则不要循环遍历搜索结果并描述它们。

```
spotify_search({"query": "miles davis kind of blue", "types": ["album"], "limit": 1})
→ 获取专辑 URI spotify:album:1weenld61qoidwYuZ1GESA
spotify_playback({"action": "play", "context_uri": "spotify:album:1weenld61qoidwYuZ1GESA"})
```

对于“播放一些 &lt;艺术家>”（无特定歌曲），优先使用 `types: ["artist"]` 并播放艺术家上下文 URI — Spotify 会处理智能随机播放。如果用户说“那首歌”或“那首曲目”，则搜索 `types: ["track"]` 并传递 `uris: [track_uri]` 进行播放。
...
如果返回 204/空（`is_playing: false`），则告诉用户没有正在播放的内容。不要重试。
...
**`403 Forbidden — No active device found`** 在任何播放操作中出现，意味着 Spotify 未在任何地方运行。告诉用户：“请先在手机/桌面/网页播放器上打开 Spotify，播放任意曲目几秒钟，然后重试。”不要盲目重试工具调用 — 它会以相同方式失败。你可以调用 `spotify_devices list` 来确认；空列表意味着没有活跃设备。

**`403 Forbidden — Premium required`** 意味着用户使用的是免费版，并尝试变更播放。不要重试；告诉他们此操作需要 Premium。读取操作仍然有效（搜索、播放列表、音乐库、获取状态）。

**`204 No Content` 在 `get_currently_playing` 上** 不是错误 — 它意味着没有正在播放的内容。工具会返回 `is_playing: false`。只需将此信息报告给用户。

**`429 Too Many Requests`** = 速率限制。等待并重试一次。如果持续发生，说明你在循环 — 停止。

**`401 Unauthorized` 在重试后** — 刷新令牌已被撤销。告诉用户重新运行 `hermes auth spotify`。

## URI 和 ID 格式

Spotify 使用三种可互换的 ID 格式。工具接受所有三种格式并进行标准化：
...
实体类型：`track`（曲目）、`album`（专辑）、`artist`（艺术家）、`playlist`（播放列表）、`show`（节目）、`episode`（单集）。为操作使用正确的类型 — `spotify_playback.play` 使用 `context_uri` 时预期专辑/播放列表/艺术家；`uris` 预期曲目 URI 数组。

## 不要做的事情

- **不要在每个操作前调用 `get_state`。** Spotify 接受播放/暂停/跳过操作而无需预检。仅在用户询问“正在播放什么”或你需要推理设备/曲目时检查状态。
...
- **不要在 `spotify_library` 中混合 `kind: "tracks"` 与专辑 URI**（反之亦然）。工具会标准化 ID，但 API 端点不同。