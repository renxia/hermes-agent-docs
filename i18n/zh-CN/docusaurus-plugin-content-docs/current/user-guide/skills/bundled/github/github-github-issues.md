---
title: "Github Issues — 通过 gh 或 REST 创建、分类、标记和分配 GitHub 问题"
sidebar_label: "Github Issues"
description: "通过 gh 或 REST 创建、分类、标记和分配 GitHub 问题"
---
...
通过 gh 或 REST 创建、分类、标记和分配 GitHub 问题。
...
创建、搜索、分类和管理 GitHub 问题。每个部分先展示 `gh` 命令，然后是 `curl` 备用方案。

## 前提条件

- 已通过 GitHub 身份验证（参见 `github-auth` 技能）
- 位于一个拥有 GitHub 远程仓库的 git 仓库内，或明确指定仓库
...
3. 登录
4. 实际结果：被重定向到 /dashboard（应跳转到 /settings）
...
```bash
gh issue comment 42 --body "已调查 — 根本原因在身份验证中间件中。正在修复。"
```
...
当 PR 合并且其描述体中包含正确的关键词时，问题会自动关闭：
