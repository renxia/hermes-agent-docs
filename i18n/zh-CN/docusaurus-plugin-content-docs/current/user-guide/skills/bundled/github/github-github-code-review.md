...
在推送前对本地更改进行代码审查，或审查 GitHub 上的开放 PR。此技能的大部分操作都使用普通的 `git` 命令 — 只有在 PR 级别交互时，`gh`/`curl` 的区别才重要。
...
2. **逐个文件审查** — 对更改的文件使用 `read_file` 以获取完整上下文，并使用 diff 查看更改内容：
...
### 建议
- **src/utils/helpers.py:8** — 与 `src/core/utils.py:34` 中的逻辑重复。请合并。
...
curl -s -X POST \
  ...
  -d "{
    ...
    \"comments\": [
      ...
      {\"path\": \"tests/test_auth.py\", \"line\": 1, \"body\": \"添加过期令牌边界情况的测试。\"}
    ]
  }"
```
...
执行代码审查（本地或 PR）时，请系统地检查：

### 正确性
- 代码是否实现了其声称的功能？
- 是否处理了边界情况（空输入、空值、大数据、并发访问）？
- 错误路径是否得到妥善处理？
...
4. 应用上述清单
5. 按结构化格式呈现结果（严重问题 / 警告 / 建议 / 良好实践）
6. 如果发现严重问题，请在用户推送前提供修复建议
...
对于每个更改的文件，使用 `read_file` 查看更改周围的完整上下文 — 仅靠差异可能会遗漏仅在周围代码中可见的问题。
...
收集你的发现，并以带行内评论的正式审查形式提交。

**使用 gh：**
```bash
# 如果无问题 — 批准
gh pr review $PR_NUMBER --approve --body "由 Hermes 智能体审查。代码看起来干净 — 测试覆盖良好，无安全问题。"
...
- **请求修改** — 存在任何需在合并前修复的严重或警告级别问题
- **评论** — 观察和建议，但无阻碍项（当不确定或PR为草稿时使用）