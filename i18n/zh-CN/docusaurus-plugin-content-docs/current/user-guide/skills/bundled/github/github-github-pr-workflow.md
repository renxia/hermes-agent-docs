...
# GitHub 拉取请求（Pull Request）工作流

管理 PR 生命周期的完整指南。每个部分首先展示 `gh` 方式，然后展示适用于没有 `gh` 的机器的 `git` + `curl` 回退方案。

## 先决条件

- 已通过 GitHub 认证（参见 `github-auth` 技能）
- 位于带有 GitHub 远程仓库的 git 仓库中
...
许多 `curl` 命令需要 `owner/repo`。请从 git 远程仓库中提取：
...
响应 JSON 包含 PR 的 `number` — 保存它以供后续命令使用。
...
```bash
# 自动合并要求仓库在设置中启用了该功能。
# 由于 REST 不支持自动合并，因此使用 GraphQL API。
PR_NODE_ID=$(curl -s \
  -H "Authorization: token $GITHUB_TOKEN" \
