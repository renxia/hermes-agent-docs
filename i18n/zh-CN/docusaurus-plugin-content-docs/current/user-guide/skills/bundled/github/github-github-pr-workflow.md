...
管理 PR 生命周期的完整指南。每个部分先展示 `gh` 的用法，然后是针对没有 `gh` 的机器的 `git` + `curl` 回退方案。

## 前提条件

- 已通过 GitHub 身份验证（参见 `github-auth` 技能）
- 位于一个具有 GitHub 远程仓库的 git 仓库内
...
# 使用约定式提交信息进行提交
git commit -m "feat: 添加基于 JWT 的用户认证
...
# 同时检查 GitHub Actions 检查运行情况（独立端点）
curl -s \
  -H "Authorization: token $GITHUB_TOKEN" \
