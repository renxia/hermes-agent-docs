...
**模式 1：** 要验证训练作业是否存在可接受的数据传输速度，运行 NCCL 测试可以帮助确定瓶颈，例如：
...
**模式 4：** 例如：- 8 个 GPU 且无序列并行：每步处理 8 个不同的批次 - 8 个 GPU 且 context_parallel_size=4：每步仅处理 2 个不同的批次（每个批次在 4 个 GPU 上拆分） - 如果您的每个 GPU 的 micro_batch_size 为 2，则全局批次大小将从 16 减少到 4

```
context_parallel_size=4
```

**模式 5：** 在您的配置中设置 save_compressed: true 可以启用以压缩格式保存模型，这将： - 减少约 40% 的磁盘空间使用 - 保持与 vLLM 的兼容性以进行加速推理 - 保持与 llmcompressor 的兼容性以进行进一步优化（例如：量化）

```
save_compressed: true
```

**模式 6：** 注意 您的集成不必放在 integrations 文件夹中。它可以位于任何位置，只要它安装在您的 python 环境的某个包中即可。请参见此仓库以获取示例：https://github.com/axolotl-ai-cloud/diff-transformer

```
integrations
```

**模式 7：** 处理单个示例和批处理数据。 - 单个示例：sample[‘input_ids’] 是一个 list[int] - 批处理数据：sample[‘input_ids’] 是一个 list[list[int]]
...
### 初学者
从 getting_started 或 tutorials 参考文件开始，以了解基本概念。

### 特定功能
使用适当的类别参考文件（api、指南等）以获取详细信息。
