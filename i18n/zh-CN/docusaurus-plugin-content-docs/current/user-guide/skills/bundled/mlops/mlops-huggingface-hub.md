...
`hf` 命令是与 Hugging Face Hub 交互的现代命令行接口，提供管理仓库、模型、数据集和 Spaces 的工具。
...
*   **身份验证：** 建议通过 `HF_TOKEN` 环境变量或 `--token` 标志进行。
...
*   **作业：** 在 HF 基础设施上运行计算任务。包括用于运行带内联依赖项的 Python 脚本的 `hf jobs uv` 和用于资源监控的 `stats`。
*   **Spaces：** 管理交互式应用。包括用于 Python 文件的 `dev-mode` 和 `hot-reload`，无需完全重启。
