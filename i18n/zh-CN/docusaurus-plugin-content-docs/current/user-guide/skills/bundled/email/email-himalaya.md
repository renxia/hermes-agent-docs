---
title: "Himalaya — Himalaya CLI：终端中的 IMAP/SMTP 邮件"
sidebar_label: "Himalaya"
description: "Himalaya CLI：终端中的 IMAP/SMTP 邮件"
---
...
Himalaya CLI：终端中的 IMAP/SMTP 邮件。
...
Himalaya 是一个 CLI 邮件客户端，允许您使用 IMAP、SMTP、Notmuch 或 Sendmail 后端从终端管理邮件。
...
1. 已安装 Himalaya CLI（使用 `himalaya --version` 验证）
...
- 使用 `--output json` 可获得更易于编程解析的结构化输出
...
要从 Hermes 非交互式地回复，请阅读原始邮件，撰写回复，并通过管道传递：
...
**非交互式（请从 Hermes 使用此方法）** — 通过 stdin 传递消息：
