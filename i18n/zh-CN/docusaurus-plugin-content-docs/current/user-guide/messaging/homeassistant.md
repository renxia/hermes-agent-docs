...
### 1. 创建长期访问令牌 (Long-Lived Access Token)

1. 打开您的 Home Assistant 实例
2. 导航到您的**个人资料**（点击侧边栏中的姓名）
3. 滚动到**长期访问令牌**
4. 点击**创建令牌**，并命名为“Hermes Agent”
5. 复制该令牌
...
:::info
当设置了 `HASS_TOKEN` 时，`homeassistant` 工具集会自动启用。网关平台和设备控制工具都通过此单个令牌激活。
:::
...
Home Assistant 将作为连接平台出现在任何其他消息平台（Telegram、Discord 等）旁边。
...
列出 Home Assistant 实体，可选择按域或区域进行过滤。
...
列出可用于设备控制的服务（动作）。显示了可以在每种设备类型上执行哪些动作以及它们接受哪些参数。
...
Home Assistant 网关适配器通过 WebSocket 连接，并订阅 `state_changed` 事件。当设备状态发生变化并匹配您的过滤器时，它会作为消息转发给 Agent。

### 事件过滤

:::warning 必需配置
默认情况下，**不会转发任何事件**。您必须配置至少一个 `watch_domains`、`watch_entities` 或 `watch_all` 才能接收事件。如果没有过滤器，系统会在启动时记录警告，并且所有状态变化都会静默丢弃。
:::

请在 `~/.hermes/config.yaml` 的 Home Assistant 平台 `extra` 部分配置 Agent 观察哪些事件：
...
| 设置 | 默认值 | 描述 |
|---------|---------|-------------|
| `watch_domains` | *(无)* | 只观察这些实体域（例如：`climate`、`light`、`binary_sensor`） |
| `watch_entities` | *(无)* | 只观察这些特定的实体 ID |
| `watch_all` | `false` | 设置为 `true` 可接收**所有**状态变化（不推荐用于大多数配置） |
| `ignore_entities` | *(无)* | 始终忽略这些实体（在域/实体过滤器之前应用） |
| `cooldown_seconds` | `30` | 同一实体事件之间的最小秒数间隔 |

:::tip
从一组集中的域开始 — `climate`、`binary_sensor` 和 `alarm_control_panel` 涵盖了最有用的自动化。根据需要添加更多。使用 `ignore_entities` 来抑制像 CPU 温度或运行时间计数器这类噪音传感器。
:::
...
Agent 发出的消息作为**Home Assistant 持久通知**（通过 `persistent_notification.create`）交付。这些通知会出现在 HA 通知面板中，标题为“Hermes Agent”。

### 连接管理

- **WebSocket**：用于实时事件的 30 秒心跳机制
- **自动重连**：带指数退避：5s → 10s → 30s → 60s
- **REST API**：用于传出通知（独立会话，避免 WebSocket 冲突）
- **授权** — HA 事件始终是授权的（不需要用户白名单，因为 `HASS_TOKEN` 负责认证连接）
...
作为网关平台连接时，Agent 可以对事件做出反应：

```
[Home Assistant] 前门: triggered (之前是 cleared)
