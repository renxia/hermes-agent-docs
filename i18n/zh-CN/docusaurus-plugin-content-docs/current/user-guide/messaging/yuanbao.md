...
sidebar_position: 16
title: "Yuanbao"
description: "通过 WebSocket 网关将 Hermes 智能体连接到 Yuanbao 企业消息平台"
---

# Yuanbao

将 Hermes 连接到 [Yuanbao](https://yuanbao.tencent.com/)，即腾讯的企业消息平台。该适配器使用 WebSocket 网关实现实时消息传递，并支持直接（C2C）和群组对话。

:::info
Yuanbao 是一个主要应用于腾讯内部和企业环境的企业消息平台。它使用 WebSocket 进行实时通信，基于 HMAC 的身份验证，并支持包括图像、文件和语音消息在内的富媒体。
:::
...
- **C2C 消息传递** — 用户与机器人之间的直接对话
- **群聊消息传递** — 群聊中的对话
- **媒体支持** — 通过 COS（云对象存储）支持图片、文件和语音消息
- **Markdown 格式化** — 消息会根据元宝的大小限制自动分块
- **消息去重** — 防止对同一消息进行重复处理
- **心跳/保活机制** — 保持 WebSocket 连接稳定
- **输入状态指示器** — 智能体处理时显示“正在输入…”状态
- **自动重连** — 处理 WebSocket 断开连接，并采用指数退避策略
- **群组信息查询** — 获取群组详情和成员列表
- **贴纸/表情支持** — 在对话中发送 TIMFaceElem 贴纸和表情
- **自动设置主频道** — 第一个向机器人发送消息的用户将被自动设为主频道所有者
- **慢响应通知** — 当智能体响应时间超过预期时，发送等待消息
...
| 群聊消息 | `group:<group_code>` | `group:grp456` |
...
媒体 URL 会在上传前自动验证并下载，以防止 SSRF 攻击。

## 主频道

在任何元宝聊天（私信或群聊）中使用 `/sethome` 命令，将其指定为**主频道**。计划任务（cron 作业）会将结果发送到该频道。

:::提示 自动设置主频道
如果未配置主频道，则第一个向机器人发送消息的用户将被自动设为主频道所有者。如果当前主频道是群聊，则第一个私信将将其升级为直接频道。
:::
...
1. 在元宝中与机器人开始对话
2. 发送命令：`/sethome`
3. 机器人回复：“主频道已设置为 [chat_name]，ID 为 [chat_id]。Cron 作业将发送到此位置。”
...
元宝有最大消息大小限制。Hermes 会自动将大型回复分块，并采用 Markdown 感知的分割方式（尊重代码围栏、表格和段落边界）。
