...
sidebar_position: 17
title: "扩展仪表板"
description: "为 Hermes Web 仪表板构建主题和插件 — 调色板、字体、布局、自定义标签页、外壳插槽、页面级插槽和后端 API 路由"
---

# 扩展仪表板

Hermes Web 仪表板 (`hermes dashboard`) 的设计目标是无需分叉代码库即可实现换肤和扩展。它暴露了三个层级：

1. **主题** — YAML 文件，用于重新定义仪表板的调色板、字体、布局以及各组件的外观。将文件放入 `~/.hermes/dashboard-themes/` 目录后，它就会出现在主题切换器中。
2. **UI 插件** — 一个包含 `manifest.json` 和 JavaScript 包的目录，用于注册标签页、替换内置页面、通过页面级插槽增强页面，或将组件注入到命名的外壳插槽中。
3. **后端插件** — 该插件目录中的一个 Python 文件，用于暴露一个 FastAPI `router`；路由挂载在 `/api/plugins/<name>/` 路径下，并由插件的 UI 调用。

这三者都支持**运行时即插即用**：无需克隆仓库、无需运行 `npm run build`、也无需修补仪表板源代码。本页是这三者的权威参考。

如果您只是想使用仪表板，请参阅 [Web 仪表板](./web-dashboard)。如果您想为终端 CLI（而非 Web 仪表板）换肤，请参阅 [皮肤与主题](./skins) — CLI 皮肤系统与仪表板主题无关。

:::note 各组件如何协同工作
主题和插件是相互独立的，但又能协同增效。一个主题可以独立存在（仅一个 YAML 文件）。一个插件也可以独立存在（仅一个标签页）。两者结合，您就可以构建一个带有自定义 HUD 的完整视觉换肤效果 — 捆绑的 `strike-freedom-cockpit` 演示正是这样做的。请参阅 [组合主题 + 插件演示](#combined-theme--plugin-demo)。
:::
...
- [主题](#themes)
  ...
  - [调色板、字体、布局](#palette-typography-layout)
  - [布局变体](#layout-variants)
  - [主题资源（将图像作为 CSS 变量）](#theme-assets-images-as-css-vars)
...
主题是存储在 `~/.hermes/dashboard-themes/` 目录下的 YAML 文件。文件名不重要（系统使用主题的 `name:` 字段），但约定为 `<name>.yaml`。每个字段都是可选的——缺失的键会回退到内置的 `default` 主题，因此一个主题可以小到只有一个颜色。
...
刷新仪表盘。点击标题栏中的调色板图标并选择 **霓虹**。背景变为黑色，文本和强调色变为洋红色，所有派生颜色（卡片、边框、柔和色、环形等）都通过 CSS 的 `color-mix()` 函数从该双色三元组重新计算得出。

这就是整个入门流程：一个文件，两种颜色。以下内容均为可选的细化配置。

### 调色板、排版、布局

这三个块是主题的核心。每个块都是独立的——可以只覆盖其中一个，保留其他部分。

#### 调色板（三层）

调色板由三层颜色、一个暖光晕染颜色和一个噪点颗粒乘数组成。仪表盘的设计系统级联通过 CSS `color-mix()` 从该三元组派生出所有与 shadcn 兼容的令牌（卡片、弹出框、柔和色、边框、主色、破坏性色、环形等）。覆盖三种颜色会级联影响整个 UI。

| 键 | 描述 |
|-----|-------------|
| `palette.background` | 最深的画布颜色——通常为接近黑色。驱动页面背景和卡片填充色。 |
| `palette.midground` | 主要文本和强调色。大多数 UI 元素（前景文本、按钮轮廓、焦点环）都使用此颜色。 |
| `palette.foreground` | 顶层高亮色。默认主题将其设置为 alpha 为 0 的白色（不可见）；希望在顶层使用明亮强调色的主题可以提高其 alpha 值。 |
| `palette.warmGlow` | `<Backdrop />` 使用的晕染颜色的 `rgba(...)` 字符串。 |
| `palette.noiseOpacity` | 颗粒覆盖层的 0–1.2 倍乘数。值越低越柔和，值越高越粗糙。 |
...
| 键 | 类型 | 描述 |
|-----|------|-------------|
| `fontSans` | 字符串 | 正文文本的 CSS 字体族堆栈（应用于 `html`、`body`）。 |
| `fontMono` | 字符串 | 代码块、`<code>`、`.font-mono` 工具类的 CSS 字体族堆栈。 |
| `fontDisplay` | 字符串 | 可选的标题/展示字体堆栈。回退到 `fontSans`。 |
| `fontUrl` | 字符串 | 可选的外部样式表 URL。在主题切换时作为 `<link rel="stylesheet">` 注入到 `<head>` 中。同一 URL 永远不会被注入两次。支持 Google Fonts、Bunny Fonts、自托管的 `@font-face` 样式表——任何可链接的资源。 |
...
| 键 | 值 | 描述 |
|-----|--------|-------------|
| `radius` | 任意 CSS 长度（`"0"`、`"0.25rem"`、`"0.5rem"`、`"1rem"`……） | 圆角令牌。映射到 `--radius` 并级联到 `--radius-sm/md/lg/xl`——所有圆角元素一起变化。 |
...
`layoutVariant` 选择整体外壳布局。缺省时默认为 `"standard"`。

| 变体 | 行为 |
|---------|-----------|
| `standard` | 单列，最大宽度 1600px（默认）。 |
| `cockpit` | 左侧边栏导轨（260px）+ 主内容。由插件通过 `sidebar` 插槽填充——参见 [外壳插槽](#shell-slots)。没有插件时导轨显示占位符。 |
...
当前变体通过 `document.documentElement.dataset.layoutVariant` 暴露，因此 `customCSS` 中的原生 CSS 可以通过 `:root[data-layout-variant="cockpit"] ...` 定位它。
...
`componentStyles` 可以在不编写 CSS 选择器的情况下重新设置单个外壳组件的样式。每个桶的条目都会变成 CSS 变量（`--component-<bucket>-<kebab-property>`），外壳的共享组件会读取这些变量。因此 `card:` 覆盖会应用于每个 `<Card>`，`header:` 会应用于应用栏，等等。
...
属性名使用 camelCase（`clipPath`），并作为 kebab（`clip-path`）发出。值是纯 CSS 字符串——任何 CSS 接受的值（`clip-path`、`border-image`、`background`、`box-shadow`、`animation`……）。

### 颜色覆盖

大多数主题都不需要此功能——三层调色板会派生出所有 shadcn 令牌。当你想要一个派生无法产生的特定强调色时（例如柔和主题的柔和破坏性红色，或品牌的特定成功绿色），请使用 `colorOverrides`。
...
每个键都 1:1 映射到 `--color-<kebab>` CSS 变量（例如 `primaryForeground` → `--color-primary-foreground`）。此处设置的任何键都会覆盖活动主题的调色板级联——切换到另一个主题会清除覆盖。

### 原生 `customCSS`

对于 `componentStyles` 无法表达的、需要选择器级别的外观（伪元素、动画、媒体查询、主题范围覆盖），请将原生 CSS 放入 `customCSS`：
...
CSS 会在主题应用时作为单个作用域的 `<style data-hermes-theme-css>` 标签注入，并在主题切换时清理。**每个主题限制为 32 KiB。**
...
| 主题 | 调色板 | 排版 | 布局 |
|-------|---------|------------|--------|
| **Hermes 蓝绿色** (`default`) | 深蓝绿色 + 奶油色 | 系统字体堆栈，15px | 0.5rem 圆角，舒适 |
| **Hermes 蓝绿色（大）** (`default-large`) | 与默认相同 | 系统字体堆栈，18px，行高 1.65 | 0.5rem 圆角，宽敞 |
| **午夜** (`midnight`) | 深蓝紫色 | Inter + JetBrains Mono，14px | 0.75rem 圆角，舒适 |
| **余烬** (`ember`) | 暖深红色 + 青铜色 | Spectral（衬线）+ IBM Plex Mono，15px | 0.25rem 圆角，舒适 |
| **单色** (`mono`) | 灰度 | IBM Plex Sans + IBM Plex Mono，13px | 0 圆角，紧凑 |
| **赛博朋克** (`cyberpunk`) | 黑色上的霓虹绿色 | 所有地方都使用 Share Tech Mono，14px | 0 圆角，紧凑 |
| **玫瑰** (`rose`) | 粉色 + 象牙色 | Fraunces（衬线）+ DM Mono，16px | 1rem 圆角，宽敞 |

引用 Google Fonts 的主题（除了 Hermes 蓝绿色）会在需要时加载样式表——你第一次切换到它们时，一个 `<link>` 标签会被注入到 `<head>` 中。

### 完整主题 YAML 参考

一个文件中的所有配置项——复制并裁剪你不需要的部分：

```yaml
# ~/.hermes/dashboard-themes/ocean.yaml
name: ocean
label: 深海
description: 深海水蓝色配珊瑚色强调
...
创建文件后刷新仪表盘。可以通过标题栏实时切换主题——点击调色板图标。选择会持久化到 `config.yaml` 的 `dashboard.theme` 下，并在重新加载时恢复。

---

## 插件

仪表板插件是一个包含 `manifest.json`、预构建的 JS 包，以及可选的 CSS 文件和带有 FastAPI 路由的 Python 文件的目录。插件与其他 Hermes 插件一起位于 `~/.hermes/plugins/<name>/` —— 仪表板扩展是该插件目录下的 `dashboard/` 子文件夹，因此一个插件可以通过单次安装同时扩展 CLI/网关和仪表板。

插件不会打包 React 或 UI 组件。它们使用暴露在 `window.__HERMES_PLUGIN_SDK__` 上的 **Plugin SDK**。这使得插件包非常小（通常只有几 KB），并避免了版本冲突。
...
刷新仪表板 —— 你的标签页将出现在导航栏中，位于 **技能** 之后。

:::tip 跳过 React.createElement
如果你更喜欢 JSX，可以使用任何打包工具（esbuild、Vite、rollup），并将 React 作为外部依赖，输出 IIFE 格式。唯一硬性要求是最终文件必须是一个可通过 `<script>` 加载的单独 JS 文件。React 永远不会被打包；它来自 `SDK.React`。
:::
...
| `slots` | 否 | 此插件填充的命名外壳插槽。**仅作为文档辅助** —— 实际注册通过 JS 包中的 `registerSlot()` 完成。在此列出插槽可使发现界面更具信息性。 |
...
| `overlay` | 固定位置层，位于其他所有内容之上。适用于 chrome（扫描线、晕影）`customCSS` 无法单独实现的效果。 |
...
如果您的插件仅增强现有页面且不需要自己的侧边栏选项卡，请将页面作用域插槽与 `tab.hidden: true` 结合使用。

Shell 仅会渲染上述插槽的 `<PluginSlot name="..." />`。注册表接受其他名称用于嵌套插件 UI —— 插件可通过 `SDK.components.PluginSlot` 暴露自己的插槽。
...
将 `tab.override` 设置为内置路由路径会使插件组件替换该页面，而不是添加新选项卡。当主题需要自定义主页 (`/`) 但希望保留仪表盘其余部分时非常有用。
...
- 路由器中位于 `/` 的原始页面组件将被移除。
- 您的插件将在 `/` 处渲染。
- 不会为 `tab.path` 添加导航选项卡（覆盖即目的）。

只有一个插件可以覆盖给定路径。如果两个插件声明相同的覆盖路径，则第一个生效，第二个将被忽略并在开发模式下发出警告。

如果您只需要向现有页面添加卡片或工具栏而无需接管整个页面，请改用[页面作用域插槽](#augmenting-built-in-pages-page-scoped-slots)。

### 增强内置页面（页面作用域插槽）

通过 `tab.override` 进行完全替换是重量级的 —— 您的插件现在拥有整个页面，包括我们未来对其进行的任何更新。大多数情况下，您只是想向现有页面添加横幅、卡片或工具栏。这正是**页面作用域插槽**的用途。

每个内置页面都会在其内容区域的顶部和底部分别暴露 `<page>:top` 和 `<page>:bottom` 插槽。您的插件通过调用 `registerSlot()` 来填充其中一个插槽 —— 内置页面继续正常工作，而您的组件将与其并排渲染。
...
- `slots` 清单字段仅用于文档说明。实际绑定发生在 JS 包中，通过 `registerSlot()` 实现。
- 多个插件可以声明相同的页面作用域插槽。它们将按注册顺序堆叠渲染。
- 当没有插件注册时零占用：内置页面将完全按以前的方式渲染。

捆绑的 `example-dashboard` 插件提供了一个实时演示，它将横幅注入到 `sessions:top` —— 安装它以查看端到端的模式。

### 仅插槽插件 (`tab.hidden`)

当 `tab.hidden: true` 时，插件会注册其组件（用于直接 URL 访问）和任何插槽，但永远不会向导航添加选项卡。由仅存在于注入插槽中的插件使用 —— 例如页眉徽标、侧边栏 HUD、覆盖层。
...
插件 API 路由绕过会话令牌认证，因为仪表盘服务器默认绑定到 localhost。**如果您运行不受信任的插件，请不要使用 `--host 0.0.0.0` 在公共接口上暴露仪表盘** —— 它们的路由也会变得可访问。
...
该文件将在插件加载时作为 `<link>` 标签注入。使用特定的类名以避免与仪表盘样式冲突，并引用仪表盘的 CSS 变量以保持主题感知：
...
仪表盘暴露每个 shadcn 令牌为 `--color-*`，以及主题额外变量（`--theme-asset-*`、`--component-<bucket>-*`、`--radius`、`--spacing-mul`）。引用这些变量，您的插件将自动根据活动主题重新换肤。
...
5. 仪表盘根据清单解析已注册组件，将选项卡添加到导航（除非 `hidden`），并将组件挂载为路由。

插件在其脚本加载后有最多 **2 秒**时间调用 `register()`。之后仪表盘将停止等待并完成初始渲染。如果插件稍后注册，它仍然会出现 —— 导航是响应式的。

如果插件的脚本加载失败（404、语法错误、IIFE 期间异常），仪表盘将在浏览器控制台中记录警告并继续运行，不包含该插件。
...
- 一个完整的主题，使用了调色板、排版、`fontUrl`、`layoutVariant: cockpit`、`assets`、`componentStyles`（带缺角的卡片、渐变背景）、`colorOverrides` 和 `customCSS`（扫描线叠加）。
- 一个仅插槽插件（`tab.hidden: true`），注册到三个插槽中：
  - `sidebar` — 一个 MS-STATUS 面板，带有由 `SDK.api.getStatus()` 驱动的实时遥测条。
  - `header-left` — 一个派系徽章，读取当前主题的 `--theme-asset-crest`。
  - `footer-right` — 一个自定义标语，替换默认的组织行。
- 插件通过 CSS 变量读取主题提供的 artwork，因此切换主题会更改英雄/徽章，而无需更改插件代码。
...
阅读插件源代码（`plugins/strike-freedom-cockpit/dashboard/dist/index.js`），了解它如何读取 CSS 变量，防止在没有插槽支持的旧版仪表板上运行，以及从一个包中注册三个插槽。
...
| 端点 | 方法 | 描述 |
|----------|--------|-------------|
| `/api/dashboard/plugins` | GET | 列出发现的插件（包含清单，不包含内部字段）。 |
...
**我的主题没有出现在选择器中。**
检查文件是否在 `~/.hermes/dashboard-themes/` 中，并且以 `.yaml` 或 `.yml` 结尾。刷新页面。运行 `curl http://127.0.0.1:9119/api/dashboard/themes` — 您的主题应该在响应中。如果 YAML 有解析错误，仪表板日志会记录在 `~/.hermes/logs/` 下的 `errors.log` 中。
...
4. 打开浏览器开发者工具 → 控制台 — 查找 IIFE 或 `window.__HERMES_PLUGINS__ is undefined` 期间的错误（表示 SDK 未初始化，通常是之前的 React 渲染崩溃）。
5. 验证您的包调用 `window.__HERMES_PLUGINS__.register(...)` 时使用的名称与 `manifest.json:name` **相同**。

**插槽注册的组件没有渲染。**
`sidebar` 插槽仅在当前主题具有 `layoutVariant: cockpit` 时渲染。其他插槽始终渲染。如果您注册到没有命中的插槽，请在 `registerSlot` 内部添加 `console.log` 以确认插件包是否运行。

**插件后端路由返回 404。**
1. 确认清单文件中有 `"api": "plugin_api.py"` 指向 `dashboard/` 内的现有文件。
2. 重启 `hermes dashboard` — 插件 API 路由在启动时挂载一次，**而不是**在重新扫描时。
3. 检查 `plugin_api.py` 是否导出模块级 `router = APIRouter()`。其他导出名称不会被识别。
4. 跟踪 `~/.hermes/logs/errors.log` 中的 `Failed to load plugin <name> API routes` — 导入错误会记录在那里。

**主题更改会丢失我的颜色覆盖。**
`colorOverrides` 的作用域是当前主题，并在主题切换时清除 — 这是设计使然。如果您希望覆盖持久化，请将它们放在主题的 YAML 中，而不是在实时切换器中。

**主题 customCSS 被截断。**
每个主题的 `customCSS` 块限制为 32 KiB。将大型样式表拆分到多个主题中，或切换到通过其 `css` 字段注入完整样式表的插件（无大小限制）。

**我想在 PyPI 上发布一个插件。**
仪表板插件是通过目录布局安装的，而不是通过 pip 入口点。目前最干净的发行路径是用户克隆到 `~/.hermes/plugins/` 的 git 仓库。目前没有为仪表板插件设置基于 pip 的安装程序。