---
title: 视觉与图像粘贴
description: 将图像从剪贴板粘贴到 Hermes CLI 中进行多模态视觉分析。
...
Hermes 智能体支持**多模态视觉**——您可以将图像直接从剪贴板粘贴到 CLI 中，并要求智能体对其进行分析、描述或处理。图像会以 base64 编码的内容块形式发送给模型，因此任何支持视觉功能的模型都可以处理它们。
...
您可以在发送前附加多个图像——每个图像都会获得自己的徽章。按 `Ctrl+C` 可清除所有已附加的图像。

图像会以带时间戳的文件名保存为 PNG 文件，存储在 `~/.hermes/images/` 目录中。

## 粘贴方法

您附加图像的方式取决于您的终端环境。并非所有方法在任何地方都有效——以下是完整说明：
...
输入 `/paste` 并按 Enter 键。Hermes 会检查您的剪贴板中是否有图像并将其附加。当您的终端重写了 `Cmd+V`/`Ctrl+V`，或者您只复制了图像而没有可检查的带括号粘贴文本负载时，这是最安全的选择。
...
当 `Cmd+Enter`、`Cmd+Z` 或 `Shift+Cmd+Z` 被 IDE 拦截时，这特别有用。请仅在本地机器上运行此命令——不要在 SSH 会话中运行。
...
| **Apple Terminal** | ✅ | ✅ | n/a | 如果 Cmd+←/→/⌫ 被重写，请使用 Ctrl+A / Ctrl+E / Ctrl+U 回退方案 |
...
**无需设置。** Hermes 使用 `osascript`（macOS 内置）读取剪贴板。为了获得更快的性能，可以选择性安装 `pngpaste`：
...
现代 Linux 桌面（Ubuntu 22.04+、Fedora 34+）通常默认使用 Wayland。请安装 `wl-clipboard`：
...
剪贴板数据会通过 stdout 以 base64 编码的 PNG 形式传输，因此无需文件路径转换或临时文件。
...
1. **上传图像文件**——将图像保存在本地，通过 `scp`、VSCode 的文件资源管理器（拖放）或任何文件传输方法将其上传到远程服务器。然后通过路径引用它。*（未来版本计划推出 `/attach <filepath>` 命令。）*
...
如果剪贴板中只包含图像（没有文本），终端就没有内容可发送。没有用于二进制图像数据的标准终端转义序列。终端 simply 什么都不做。
...
## 图像路由（支持视觉功能的模型 vs 纯文本模型）

当用户附加图像时——无论是来自 CLI 剪贴板、网关（Telegram/Discord 照片）还是任何其他入口点——Hermes 会根据您当前模型是否实际支持视觉功能来路由它：

| 您的模型 | 图像的处理方式 |
|---|---|
| **支持视觉功能**（GPT-4V、带视觉功能的 Claude、Gemini、Qwen-VL、MiMo-VL 等） | 使用上述提供商的本地图像内容格式作为**真实像素**发送。无文本摘要层。 |
| **纯文本**（DeepSeek V3、较小的开源模型、旧版仅聊天端点） | 通过 `vision_analyze` 辅助工具路由——辅助视觉模型描述图像，文本描述会被注入对话中。 |

您无需配置此功能——Hermes 会在提供商元数据中查找您当前模型的功能，并自动选择正确的路径。实际效果：您可以在会话中途切换支持视觉功能和不支持视觉功能的模型，而图像处理“just works”，无需更改您的工作流程。纯文本模型会获得关于图像的连贯上下文，而不是它们必须拒绝的损坏多模态负载。
