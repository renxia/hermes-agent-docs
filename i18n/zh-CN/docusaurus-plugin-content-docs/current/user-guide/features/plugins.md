...
默认情况下，位于 `./.hermes/plugins/` 下的项目本地插件是禁用的。仅在信任的仓库中，通过在启动 Hermes 之前设置 `HERMES_ENABLE_PROJECT_PLUGINS=true` 来启用它们。
...
| 项目 | `.hermes/plugins/` | 项目专用插件（需要设置 `HERMES_ENABLE_PROJECT_PLUGINS=true`） |
...
后面的来源会覆盖前面的同名插件，因此用户插件如果与内置插件同名，则会替换内置插件。

## 插件默认不启用

**所有插件 — 用户安装的、内置的或通过 pip 安装的 — 默认都是禁用的。** 发现机制会找到它们（因此它们会出现在 `hermes plugins` 和 `/plugins` 中），但除非你将插件名称添加到 `~/.hermes/config.yaml` 中的 `plugins.enabled`，否则不会加载任何插件。这可以防止任何带有钩子或工具的插件在未经你明确同意的情况下运行。
...
在 `hermes plugins install owner/repo` 之后，系统会询问 `立即启用 'name' 吗？[y/N]` — 默认为否。使用 `--enable` 或 `--no-enable` 可以在脚本化安装时跳过提示。

### 现有用户的迁移

当你升级到具有“选择启用”插件功能的 Hermes 版本（配置 schema v21+）时，任何已经安装在 `~/.hermes/plugins/` 下且尚未在 `plugins.disabled` 中的用户插件都会**自动继承**到 `plugins.enabled` 中。你现有的设置将继续工作。内置插件不会自动继承 — 即使是现有用户也必须明确选择启用。

## 可用的钩子

插件可以为这些生命周期事件注册回调函数。有关完整详细信息、回调签名和示例，请参阅 **[事件钩子页面](/docs/user-guide/features/hooks#plugin-hooks)**。
...
| [`on_session_end`](/docs/user-guide/features/hooks#on_session_end) | 每次 `run_conversation` 调用结束时 + CLI 退出处理程序 |
| [`pre_gateway_dispatch`](/docs/user-guide/features/hooks#pre_gateway_dispatch) | 网关收到用户消息后，在认证和分发之前。返回 `{"action": "skip" \| "rewrite" \| "allow", ...}` 来影响流程。 |
...
记忆提供者和上下文引擎属于**提供者插件** — 每种类型只能有一个处于激活状态。通用插件可以任意组合启用。

## NixOS 声明式插件

在 NixOS 上，可以通过模块选项声明式地安装插件 — 无需 `hermes plugins install`。有关完整详细信息，请参阅 **[Nix 配置指南](/docs/getting-started/nix-setup#plugins)**。
...
声明式插件会带有 `nix-managed-` 前缀创建符号链接 — 它们与手动安装的插件共存，并在从 Nix 配置中移除时自动清理。
...
- **通用插件部分** — 复选框，用空格键切换。勾选 = 在 `plugins.enabled` 中，未勾选 = 在 `plugins.disabled` 中（明确关闭）。
...
新安装或内置插件的默认状态是 `not enabled`。`hermes plugins list` 会显示所有三种不同的状态，以便你可以区分哪些是明确关闭的，哪些只是等待启用。
...
- 如果智能体处于**空闲**状态（等待用户输入），消息将作为下一个输入排队，并开始新一轮。
- 如果智能体处于**进行中**状态（正在运行），消息将中断当前操作 — 就像用户输入新消息并按回车键一样。
...
有关处理程序约定、schema 格式、钩子行为、错误处理和常见错误的完整指南，请参阅 **[完整指南](/docs/guides/build-a-hermes-plugin)**。