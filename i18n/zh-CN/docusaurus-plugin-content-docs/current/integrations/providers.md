...
此页面介绍如何为 Hermes 智能体设置推理提供商，包括 OpenRouter 和 Anthropic 等云 API、Ollama 和 vLLM 等自托管端点，以及高级路由和故障转移配置。要使用 Hermes，您至少需要配置一个提供商。

## 推理提供商

你需要至少一种连接大语言模型（LLM）的方式。使用 `hermes model` 命令以交互方式切换提供商和模型，或直接进行配置：
...
| **Anthropic** | `hermes model`（通过 Claude Code 认证、Anthropic API 密钥或手动设置令牌访问 Claude Pro/Max） |
...
:::tip 模型键别名
在 `model:` 配置部分中，你可以使用 `default:` 或 `model:` 作为模型 ID 的键名。`model: { default: my-model }` 和 `model: { model: my-model }` 效果完全相同。
:::

### 通过 OAuth 使用 Google Gemini（`google-gemini-cli`）

`google-gemini-cli` 提供商使用 Google 的 Cloud Code Assist 后端——与 Google 自家 `gemini-cli` 工具使用的 API 相同。该方式同时支持**免费层级**（个人账户每日配额慷慨）和**付费层级**（通过 GCP 项目访问 Standard/Enterprise）。
...
# → 查看政策警告，确认
# → 浏览器跳转至 accounts.google.com，登录
# → 完成 — Hermes 在首次请求时自动为你开通免费层级
```

Hermes 默认内置 Google 的**公开** `gemini-cli` 桌面 OAuth 客户端——与 Google 在其开源 `gemini-cli` 中包含的凭据完全相同。桌面 OAuth 客户端并非机密（PKCE 提供安全保障）。你无需安装 `gemini-cli` 或注册自己的 GCP OAuth 客户端。

**认证流程：**
- 针对 `accounts.google.com` 的 PKCE 授权码流程
- 浏览器回调地址为 `http://127.0.0.1:8085/oauth2callback`（若端口被占用则回退至临时端口）
- 令牌存储在 `~/.hermes/auth/google_oauth.json`（权限 chmod 0600，原子写入，跨进程 `fcntl` 锁）
- 在过期前 60 秒自动刷新
- 无头环境（SSH、`HERMES_HEADLESS=1`）→ 回退至粘贴模式
...
- 类 OpenAI 格式的 `messages[]`、`tools[]`、`tool_choice` 被转换为 Gemini 原生格式
  `contents[]`、`tools[].functionDeclarations`、`toolConfig`
- 响应再转换回 OpenAI 格式，以便 Hermes 其余部分无需改动即可工作

**层级与项目 ID：**

| 你的情况 | 操作建议 |
|---|---|
| 个人 Google 账户，希望使用免费层级 | 无需操作 — 登录并开始对话 |
| 工作区 / Standard / Enterprise 账户 | 设置 `HERMES_GEMINI_PROJECT_ID` 或 `GOOGLE_CLOUD_PROJECT` 为你的 GCP 项目 ID |
...
:::warning 政策风险
Google 认为将 Gemini CLI OAuth 客户端用于第三方软件属于政策违规。部分用户报告了账户受限的情况。为获得最低风险体验，请使用你自己的 API 密钥并通过 `gemini` 提供商访问。Hermes 会在 OAuth 开始前显示前置警告并要求明确确认。
:::

**自定义 OAuth 客户端（可选）：**

如果你更愿意注册自己的 Google OAuth 客户端——例如，将配额和授权范围限定到你自己的 GCP 项目——请设置：
...
:::info Codex 说明
OpenAI Codex 提供商通过设备码认证（打开 URL，输入验证码）。Hermes 将生成的凭据存储在其自身的认证存储中（`~/.hermes/auth.json`），并可在存在时从 `~/.codex/auth.json` 导入现有的 Codex CLI 凭据。无需安装 Codex CLI。
:::

:::warning
即使使用 Nous Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）也会使用单独的“辅助”模型——默认为通过 OpenRouter 的 Gemini Flash。设置 `OPENROUTER_API_KEY` 可自动启用这些工具。你也可以配置这些工具使用的模型及提供商——参见[辅助模型](/docs/user-guide/configuration#auxiliary-models)。
:::

:::tip Nous 工具网关
付费 Nous Portal 订阅用户还可访问 **[工具网关](/docs/user-guide/features/tool-gateway)** —— 网络搜索、图像生成、TTS 和浏览器自动化均通过你的订阅路由。无需额外 API 密钥。该功能会在 `hermes model` 设置期间自动提供，或稍后通过 `hermes tools` 启用。
:::
...
| **`/model`** | 在 Hermes 聊天会话内部 | 快速切换**已配置**的提供商和模型 |

如果你试图切换到一个尚未设置的提供商（例如，你只配置了 OpenRouter 但想使用 Anthropic），你需要使用 `hermes model`，而不是 `/model`。先退出当前会话（`Ctrl+C` 或 `/quit`），运行 `hermes model`，完成提供商设置，然后启动新会话。
...
# 若可用，Hermes 将直接使用 Claude Code 的凭据存储
hermes model

# 手动覆盖（使用 setup-token，备用 / 旧版）
...
当你通过 `hermes model` 选择 Anthropic OAuth 时，Hermes 优先使用 Claude Code 自身的凭据存储，而非将令牌复制到 `~/.hermes/.env`。这确保了可刷新的 Claude 凭据保持可刷新状态。
...
| 细粒度 PAT | `github_pat_` | GitHub 设置 → 开发者设置 → 细粒度令牌（需**Copilot 请求**权限） |
...
Hermes 将支持的 GitHub 令牌（`gho_*`、`github_pat_*` 或 `ghu_*`）直接发送至 `api.githubcopilot.com`，并包含 Copilot 专用头部（`Editor-Version`、`Copilot-Integration-Id`、`Openai-Intent`、`x-initiator`）。

在收到 HTTP 401 时，Hermes 现在会在回退前执行一次性凭据恢复：

1. 通过正常优先级链重新解析令牌（`COPILOT_GITHUB_TOKEN` → `GH_TOKEN` → `GITHUB_TOKEN` → `gh auth token`）
2. 使用刷新后的头部重建共享 OpenAI 客户端
3. 重试请求一次

一些较旧的社区代理使用 `api.github.com/copilot_internal/v2/token` 交换流程。该端点对某些账户类型可能不可用（返回 404）。因此，Hermes 将直接令牌认证作为主要路径，并依赖运行时凭据刷新 + 重试以确保鲁棒性。
:::

**API 路由**：除 `gpt-5-mini` 外的 GPT-5+ 模型自动使用 Responses API。所有其他模型（GPT-4o、Claude、Gemini 等）使用 Chat Completions。模型会从实时 Copilot 目录中自动检测。

**`copilot-acp` — Copilot ACP 智能体后端**。将本地 Copilot CLI 作为子进程启动：

```bash
hermes chat --provider copilot-acp --model copilot-acp
# 要求 PATH 中有 GitHub Copilot CLI 且已存在 `copilot login` 会话
```
...
这些提供商具有内置支持，并拥有专用提供商 ID。设置 API 密钥并使用 `--provider` 选择：
...
可通过 `GLM_BASE_URL`、`KIMI_BASE_URL`、`MINIMAX_BASE_URL`、`MINIMAX_CN_BASE_URL`、`DASHSCOPE_BASE_URL`、`XIAOMI_BASE_URL`、`GMI_BASE_URL` 或 `TOKENHUB_BASE_URL` 环境变量覆盖基础 URL。

:::note Z.AI 端点自动检测
使用 Z.AI / GLM 提供商时，Hermes 会自动探测多个端点（全球、中国、编程变体）以找到接受你 API 密钥的端点。你无需手动设置 `GLM_BASE_URL` —— 工作端点会被自动检测并缓存。
:::

### xAI（Grok）— Responses API + 提示缓存

xAI 通过 Responses API（`codex_responses` 传输）连接，以便在 Grok 4 模型上自动支持推理 —— 无需 `reasoning_effort` 参数，服务器默认进行推理。在 `~/.hermes/.env` 中设置 `XAI_API_KEY` 并在 `hermes model` 中选择 xAI，或在 `/model` 中使用 `grok` 作为快捷方式切换至 `grok-4-1-fast-reasoning`。

当使用 xAI 作为提供商（任何包含 `x.ai` 的基础 URL）时，Hermes 会自动启用提示缓存，通过在每次 API 请求中发送 `x-grok-conv-id` 头部实现。这会将请求路由至同一会话内的同一服务器，使 xAI 基础设施能够重用缓存的系统提示和对话历史。

无需配置 —— 当检测到 xAI 端点且会话 ID 可用时，缓存会自动激活。这降低了多轮对话的延迟和成本。

xAI 还提供专用 TTS 端点（`/v1/tts`）。在 `hermes tools` → 语音 & TTS 中选择 **xAI TTS**，或查看 [语音 & TTS](../user-guide/features/tts.md#text-to-speech) 页面进行配置。

### Ollama Cloud — 托管 Ollama 模型，OAuth + API 密钥

[Ollama Cloud](https://ollama.com/cloud) 托管与本地 Ollama 相同的开源权重目录，但无需 GPU 要求。在 `hermes model` 中选择 **Ollama Cloud**，粘贴你在 [ollama.com/settings/keys](https://ollama.com/settings/keys) 获取的 API 密钥，Hermes 会自动发现可用模型。
...
两者使用相同的 OpenAI 兼容 API。Cloud 是一等提供商（`--provider ollama-cloud`，`OLLAMA_API_KEY`）；本地 Ollama 通过自定义端点流程访问（基础 URL `http://localhost:11434/v1`，无需密钥）。对于无法本地运行的大型模型使用 cloud；对于隐私或离线工作使用本地。
:::

### AWS Bedrock

通过 AWS Bedrock 使用 Anthropic Claude、Amazon Nova、DeepSeek v3.2、Meta Llama 4 等模型。使用 AWS SDK（`boto3`）凭据链 —— 无需 API 密钥，仅需标准 AWS 认证。
...
认证使用标准 boto3 链：显式 `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`、来自 `~/.aws/credentials` 的 `AWS_PROFILE`、EC2/ECS/Lambda 上的 IAM 角色、IMDS 或 SSO。如果你已通过 AWS CLI 认证，则无需设置环境变量。

Bedrock 底层使用 **Converse API** —— 请求被转换为 Bedrock 的与模型无关的格式，因此相同配置适用于 Claude、Nova、DeepSeek 和 Llama 模型。仅当你调用非默认区域端点时才需设置 `BEDROCK_BASE_URL`。

参见 [AWS Bedrock 指南](/docs/guides/aws-bedrock) 了解 IAM 设置、区域选择和跨区域推理的详细步骤。
...
仅当门户端点迁移时才需设置 `HERMES_QWEN_BASE_URL`（默认：`https://portal.qwen.ai/v1`）。

:::tip Qwen OAuth 与 DashScope（阿里云）对比
`qwen-oauth` 使用面向消费者的 Qwen Portal 及 OAuth 登录 —— 适合个人用户。`alibaba` 提供商使用 DashScope 的企业 API 及 `DASHSCOPE_API_KEY` —— 适合编程 / 生产工作负载。两者均路由至 Qwen 系列模型，但位于不同端点。
:::

### NVIDIA NIM

通过 [build.nvidia.com](https://build.nvidia.com)（免费 API 密钥）或本地 NIM 端点使用 Nemotron 等开源模型。
...
:::tip 本地 NIM
对于本地部署（DGX Spark、本地 GPU），设置 `NVIDIA_BASE_URL=http://localhost:8000/v1`。NIM 暴露与 build.nvidia.com 相同的 OpenAI 兼容聊天补全 API，因此云与本地之间的切换只需更改一行环境变量。
:::

### Hugging Face 推理提供商

[Hugging Face 推理提供商](https://huggingface.co/docs/inference-providers) 通过统一的 OpenAI 兼容端点（`router.huggingface.co/v1`）路由至 20 多个开源模型。请求会自动路由至最快可用后端（Groq、Together、SambaNova 等），并支持自动故障转移。
...
在 [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens) 获取你的令牌 —— 确保启用“调用推理提供商”权限。包含免费层级（每月 $0.10 额度，提供商费率无加价）。
...
Hermes Agent 可与**任何兼容 OpenAI 的 API 端点**配合使用。只要服务器实现了 `/v1/chat/completions` 接口，您就可以将 Hermes 指向该服务器。这意味着您可以使用本地模型、GPU 推理服务器、多提供商路由器或任何第三方 API。
...
:::warning 旧版环境变量
`.env` 中的 `OPENAI_BASE_URL` 和 `LLM_MODEL` 已**移除**。Hermes 的任何部分都不会读取这两个变量——`config.yaml` 是模型与端点配置的唯一可信来源。如果您在 `.env` 中有过时的条目，它们将在下次运行 `hermes setup` 或配置迁移时自动清除。请使用 `hermes model` 或直接编辑 `config.yaml`。
:::
...
**`hermes model`**（在终端中运行，不在任何聊天会话内）是**完整的提供商设置向导**。使用它来添加新提供商、运行 OAuth 流程、输入 API 密钥以及配置自定义端点。

**`/model`**（在活跃的 Hermes 聊天会话中输入）只能**在您已设置的提供商和模型之间切换**。它无法添加新提供商、运行 OAuth 或提示输入 API 密钥。如果您只配置了一个提供商（例如 OpenRouter），`/model` 将仅显示该提供商的模型。
...
切换提供商时，Hermes 会将基础 URL 和提供商信息持久化到配置中，因此更改在重启后仍然有效。当从自定义端点切换回内置提供商时，过时的基础 URL 会被自动清除。

:::tip
`/model custom`（裸命令，无模型名）会查询您端点的 `/models` API，并在仅加载一个模型时自动选择该模型。适用于运行单一模型的本地服务器。
:::
...
[Ollama](https://ollama.com/) 只需一条命令即可在本地运行开源权重模型。最适合：快速本地实验、隐私敏感工作、离线使用。通过兼容 OpenAI 的 API 支持工具调用。
...
对于使用工具的**智能体**，**您需要至少 16k–32k 的上下文**。在 4k 情况下，仅系统提示词 + 工具 schema 就可能填满窗口，导致对话无空间可用。
...
[vLLM](https://docs.vllm.ai/) 是生产环境大模型服务的标准。最适合：在 GPU 硬件上实现最大吞吐量、服务大型模型、连续批处理。
...
# 跳过 API 密钥（如果您为 vLLM 配置了 --api-key，则输入）
# 输入模型名称：meta-llama/Llama-3.1-70B-Instruct
```

**上下文长度：** vLLM 默认读取模型的 `max_position_embeddings`。如果该值超出 GPU 内存，会报错并要求您设置更低的 `--max-model-len`。您也可以使用 `--max-model-len auto` 自动找到适合的最大值。设置 `--gpu-memory-utilization 0.95`（默认 0.9）可在显存中容纳更多上下文。
...
支持的解析器：`hermes`（Qwen 2.5、Hermes 2/3）、`llama3_json`（Llama 3.x）、`mistral`、`deepseek_v3`、`deepseek_v31`、`xlam`、`pythonic`。缺少这些标志时，工具调用将无法工作——模型会将工具调用输出为文本。
...
**上下文长度：** SGLang 默认从模型配置中读取。使用 `--context-length` 覆盖。如果需要超过模型声明的最大值，请设置 `SGLANG_ALLOW_OVERWRITE_LONGER_CONTEXT_LEN=1`。

**工具调用：** 使用 `--tool-call-parser` 并指定适合您模型家族的解析器：`qwen`（Qwen 2.5）、`llama3`、`llama4`、`deepseekv3`、`mistral`、`glm`。缺少此标志时，工具调用将以纯文本形式返回。

:::caution SGLang 默认最大输出 token 数为 128
如果响应似乎被截断，请在请求中添加 `max_tokens` 或在服务器上设置 `--default-max-tokens`。如果请求中未指定，SGLang 默认每响应仅 128 个 token。
:::
...
**上下文长度（`-c`）：** 近期构建版本默认为 `0`，表示从 GGUF 元数据中读取模型的训练上下文。对于训练上下文为 128k+ 的模型，这可能导致尝试分配完整 KV 缓存时内存不足（OOM）。请显式设置 `-c` 为您所需的值（智能体使用建议范围为 32k–64k）。如果使用并行槽位（`-np`），总上下文将在槽位间分配——例如 `-c 32768 -np 4` 时，每个槽位仅获得 8k。
...
若不使用 `--jinja`，llama-server 将完全忽略 `tools` 参数。模型会尝试通过在响应文本中写入 JSON 来调用工具，但 Hermes 无法将其识别为工具调用——你会看到类似 `{"name": "web_search", ...}` 的原始 JSON 作为消息打印出来，而不是实际执行搜索。

原生工具调用支持（性能最佳）：Llama 3.x、Qwen 2.5（包括 Coder）、Hermes 2/3、Mistral、DeepSeek、Functionary。其他所有模型使用通用处理程序，虽然可用但效率可能较低。完整列表请参见 [llama.cpp 函数调用文档](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md)。

你可以通过访问 `http://localhost:8080/props` 验证工具调用是否启用——`chat_template` 字段应存在。
:::
...
[LM Studio](https://lmstudio.ai/) 是一个用于运行本地模型并带有图形用户界面的桌面应用。最适合：偏好可视化界面的用户、快速测试模型、macOS/Windows/Linux 开发者。
...
你可以使用 CLI 预估模型是否能适配：`lms load model-name --context-length 64000 --estimate-only`

要设置持久的每个模型默认值：我的模型选项卡 → 点击模型上的齿轮图标 → 设置上下文大小。
:::
...
由于 Hermes 智能体需要 Unix 环境，Windows 用户需在 WSL2 中运行它。如果你的模型服务器（Ollama、LM Studio 等）运行在 **Windows 主机**上，你需要弥合网络差距——WSL2 使用带有自己子网的虚拟网络适配器，因此 WSL2 内的 `localhost` 指的是 Linux 虚拟机，**而非** Windows 主机。

:::tip 都在 WSL2 中？没问题。
如果你的模型服务器也运行在 WSL2 内（vLLM、SGLang 和 llama-server 常见），`localhost` 按预期工作——它们共享相同的网络命名空间。跳过本节。
:::
...
:::note Hyper-V 防火墙
在某些 Windows 11 版本中，Hyper-V 防火墙默认阻止镜像连接。如果在启用镜像模式后 `localhost` 仍无法工作，请在 **管理员 PowerShell** 中运行以下命令：
```powershell
...
如果无法使用镜像模式，请从 WSL2 内部获取 Windows 主机 IP 并替代 `localhost`：
...
:::tip 动态助手
主机 IP 可能在 WSL2 重启时变化。你可以在 shell 中动态获取它：
```bash
...
或使用你的机器的 mDNS 名称（WSL2 中需安装 `libnss-mdns`）：
```bash
...
如果你使用 **选项 2**（使用主机 IP 的 NAT 模式），Windows 上的模型服务器必须接受来自 `127.0.0.1` 外部的连接。默认情况下，大多数服务器仅监听本地主机——NAT 模式下的 WSL2 连接来自不同的虚拟子网，将被拒绝。在镜像模式下，`localhost` 直接映射，因此默认的 `127.0.0.1` 绑定可以正常工作。
...
Windows 防火墙将 WSL2 视为独立网络（NAT 和镜像模式均如此）。如果在上述步骤后连接仍失败，请为模型服务器的端口添加防火墙规则：
...
如果收到列出模型的 JSON 响应，则一切正常。在 Hermes 配置中使用相同的 URL 作为 `base_url`。
...
如果你在 WSL2 内运行 Hermes，而模型服务器运行在 Windows 主机上，在 WSL2 默认的 NAT 网络模式下，`http://localhost:<port>` 将无法工作。修复方法见上文的 [WSL2 网络](#wsl2-networking-windows-users)。
...
**原因：** 上下文窗口太小。当对话超过上下文限制时，大多数服务器会静默丢弃较早的消息。仅 Hermes 的系统提示 + 工具架构就可能占用 4k–8k 个 token。
...
**修复：** 为智能体使用将上下文设置为至少 **32,768 个 token**。具体标志请参见上文各服务器部分。

#### 启动时显示“Context limit: 2048 tokens”

Hermes 会从服务器的 `/v1/models` 端点自动检测上下文长度。如果服务器报告的值较低（或完全不报告），Hermes 将使用模型声明的限制，这可能是不正确的。
...
**可能的原因：**
1. **服务器输出上限（`max_tokens`）过低** —— SGLang 默认每次响应生成 128 个 token。请在服务器上设置 `--default-max-tokens`，或在 config.yaml 中通过 `model.max_tokens` 配置 Hermes。注意：`max_tokens` 仅控制响应长度，与对话历史长度无关（后者由 `context_length` 控制）。
...
[LiteLLM](https://docs.litellm.ai/) 是一个兼容 OpenAI 的代理，可将 100 多个大语言模型（LLM）提供商统一至单一 API 接口。适用场景：无需更改配置即可切换提供商、负载均衡、备用链路、预算控制。
...
:::note
ClawRouter 需要您在 Base 或 Solana 网络上拥有一个已充值 USDC 的钱包用于支付。所有请求均通过 BlockRun 的后端 API 路由。运行 `npx @blockrun/clawrouter doctor` 可检查钱包状态。
:::
...
:::note 两个易混淆的设置
**`context_length`** 表示**总上下文窗口** —— 输入 token 与输出 token 的合计预算（例如 Claude Opus 4.6 为 200,000）。Hermes 据此决定何时压缩历史记录，并验证 API 请求。

**`model.max_tokens`** 表示**输出上限** —— 模型在*单次响应*中最多可生成的 token 数量。它与对话历史长度无关。行业标准名称 `max_tokens` 常引起混淆；Anthropic 的原生 API 已将其更名为 `max_output_tokens` 以提高清晰度。

当自动检测无法正确识别窗口大小时，请手动设置 `context_length`。  
仅当您需限制单次响应长度时，才设置 `model.max_tokens`。
:::
...
8. **[models.dev](https://models.dev)** —— 社区维护的注册表，涵盖 100+ 提供商下 3800+ 模型的提供商特定上下文长度
9. **后备默认值** —— 广义模型家族模式（默认 128K）

对于大多数配置，此机制开箱即用。系统具备提供商感知能力 —— 同一模型在不同服务商处可能有不同的上下文限制（例如 `claude-opus-4.6` 在 Anthropic 直接访问时为 1M，但在 GitHub Copilot 上为 128K）。
...
也可通过交互式 `hermes model` 菜单选择命名的自定义提供商。
...
| **本地模型，简易部署** | Ollama |
| **生产级 GPU 推理服务** | vLLM 或 SGLang |
| **Mac / 无 GPU** | Ollama 或 llama.cpp |
| **多提供商路由** | LiteLLM Proxy 或 OpenRouter |
| **成本优化** | ClawRouter 或 OpenRouter（设置 `sort: "price"`） |
| **最高隐私要求** | Ollama、vLLM 或 llama.cpp（完全本地运行） |
| **企业 / Azure** | Azure OpenAI 配合自定义端点 |
| **中文 AI 模型** | z.ai（GLM）、Kimi/Moonshot（`kimi-coding` 或 `kimi-coding-cn`）、MiniMax、小米 MiMo 或腾讯 TokenHub（均为一级支持提供商） |
...
默认情况下，Hermes 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网页搜索和抓取。如果您更愿意在本地运行 Firecrawl，也可以将 Hermes 指向一个自托管实例。请参阅 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md) 以获取完整的设置说明。

**您将获得：** 无需 API 密钥，无速率限制，无单页成本，完全的数据主权。
...
配置一个备份提供商:模型，当您的主模型失败时（速率限制、服务器错误、身份验证失败），Hermes 会自动切换到该备用模型：
...
激活后，备用模型会在会话中途交换模型和提供商，而不会丢失您的对话。每个会话最多触发**一次**。

支持的提供商：`openrouter`、`nous`、`openai-codex`、`copilot`、`copilot-acp`、`anthropic`、`gemini`、`google-gemini-cli`、`qwen-oauth`、`huggingface`、`zai`、`kimi-coding`、`kimi-coding-cn`、`minimax`、`minimax-cn`、`deepseek`、`nvidia`、`xai`、`ollama-cloud`、`bedrock`、`ai-gateway`、`opencode-zen`、`opencode-go`、`kilocode`、`xiaomi`、`arcee`、`gmi`、`alibaba`、`tencent-tokenhub`、`custom`。

:::tip
备用模型仅通过 `config.yaml` 配置 —— 没有与之相关的环境变量。有关其触发时机、支持的提供商以及它如何与辅助任务和委派交互的详细信息，请参阅 [备用提供商](/docs/user-guide/features/fallback-providers)。
:::
