...
sidebar_label: "构建插件"
title: "构建 Hermes 插件"
description: "分步指南：使用工具、钩子、数据文件和技能构建完整的 Hermes 插件"
---

# 构建 Hermes 插件

本指南将引导您从头开始构建一个完整的 Hermes 插件。最终您将拥有一个功能齐全的插件，包含多个工具、生命周期钩子、附带的数据文件以及捆绑的技能——涵盖插件系统支持的所有功能。
...
这会告诉 Hermes：“我是一个名为 calculator 的插件，我提供工具和钩子。” `provides_tools` 和 `provides_hooks` 字段是插件注册内容的列表。
...
:::tip 传统模式
旧的 `shutil.copy2` 模式（将技能复制到 `~/.hermes/skills/` 中）仍然有效，但会与内置技能产生名称冲突的风险。新的插件推荐使用 `ctx.register_skill()`。
:::
...
如果 `WEATHER_API_KEY` 未设置，插件会被禁用，并显示一条清晰的消息。不会崩溃，智能体中也不会出错 — 只是“插件 weather 被禁用（缺失：WEATHER_API_KEY）”。

当用户运行 `hermes plugins install` 时，他们会**交互式地提示**输入任何缺失的 `requires_env` 变量。值会自动保存到 `.env`。
...
每个钩子在**[事件钩子参考](/docs/user-guide/features/hooks#plugin-hooks)**中有完整文档 — 回调签名、参数表、每个钩子触发的确切时间以及示例。以下是摘要：
...
| [`on_session_finalize`](/docs/user-guide/features/hooks#on_session_finalize) | CLI/网关拆除一个活跃会话 | `session_id: str \| None, platform: str` | 忽略 |
| [`on_session_reset`](/docs/user-guide/features/hooks#on_session_reset) | 网关交换一个新的会话键 (`/new`, `/reset`) | `session_id: str, platform: str` | 忽略 |

大多数钩子都是即发即忘的观察者 — 它们的返回值会被忽略。例外是 `pre_llm_call`，它可以将上下文注入到对话中。

所有回调都应该接受 `**kwargs` 以保证向前兼容性。如果钩子回调崩溃，它会被记录并跳过。其他钩子和智能体会继续正常运行。

### `pre_llm_call` 上下文注入

这是唯一一个返回值重要的钩子。当 `pre_llm_call` 回调返回一个包含 `"context"` 键的字典（或一个纯字符串）时，Hermes 会将该文本注入到**当前轮次的用户消息**中。这是记忆插件、RAG 集成、防护栏以及任何需要为模型提供额外上下文的插件的机制。
...
任何非 None、非空的返回，如果包含 `"context"` 键（或一个纯非空字符串），都会被收集并附加到当前轮次的用户消息中。

#### 注入如何工作

注入的上下文会被附加到**用户消息**，而不是系统提示。这是一个刻意的设计选择：

- **提示缓存保留** — 系统提示在轮次之间保持相同。Anthropic 和 OpenRouter 会缓存系统提示前缀，因此保持其稳定可以在多轮对话中节省 75% 以上的输入 token。如果插件修改了系统提示，每一轮都会是缓存未命中。
- **短暂性** — 注入仅在 API 调用时发生。对话历史中的原始用户消息永远不会被修改，也不会有任何内容持久化到会话数据库。
- **系统提示是 Hermes 的领域** — 它包含特定于模型的指导、工具执行规则、个性指令和缓存的技能内容。插件通过在用户输入旁边提供上下文来贡献上下文，而不是通过改变智能体的核心指令。
...
当多个插件从 `pre_llm_call` 返回上下文时，它们的输出会用双换行符连接，并一起附加到用户消息中。顺序遵循插件发现顺序（按插件目录名称的字母顺序）。

### 注册 CLI 命令

插件可以添加它们自己的 `hermes <plugin>` 子命令树：
...
**记忆提供者插件**使用基于约定的方法：在你的插件的 `cli.py` 文件中添加一个 `register_cli(subparser)` 函数。记忆插件发现系统会自动找到它 — 不需要调用 `ctx.register_cli_command()`。有关详细信息，请参阅[记忆提供者插件指南](/docs/developer-guide/memory-provider-plugin#adding-cli-commands)。

**活跃提供者控制：** 记忆插件 CLI 命令仅在它们的提供者是配置中的活跃 `memory.provider` 时才会出现。如果用户没有设置你的提供者，你的 CLI 命令不会使帮助输出混乱。

### 注册斜杠命令

插件可以注册会话内斜杠命令 — 用户在对话期间输入的命令（如 `/lcm status` 或 `/ping`）。这些在 CLI 和网关（Telegram、Discord 等）中都有效。
...
| 有效位置 | CLI 会话、Telegram、Discord 等 | 仅终端 |
| 处理器接收 | 原始参数字符串 | argparse `Namespace` |
| 用例 | 诊断、状态、快速操作 | 复杂子命令树、设置向导 |

**冲突保护：** 如果插件尝试注册与内置命令（`help`, `model`, `new` 等）冲突的名称，注册会被静默拒绝，并记录警告。内置命令始终优先。
...
:::tip
本指南涵盖了**通用插件**（工具、钩子、斜杠命令、CLI 命令）。对于专门的插件类型，请参阅：
...
如果你的插件提供了带有入口点的 `pyproject.toml`，NixOS 用户可以声明性地安装它：
...
有关包括覆盖层使用和冲突检查在内的完整文档，请参阅[Nix 设置指南](/docs/getting-started/nix-setup#plugins)。
...
# 好 — 模型确切知道何时以及如何使用
"description": "计算一个数学表达式。用于算术、三角函数、对数。支持：+、-、*、/、**、sqrt、sin、cos、log、pi、e。"
```