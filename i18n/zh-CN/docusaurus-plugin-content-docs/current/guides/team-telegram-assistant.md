...
智能体会自动创建一个定时任务，并将结果发送到您提问的聊天（或主频道）。
...
:::warning
定时任务提示会在完全独立的新会话中运行，不会保留先前的对话记忆。请确保每个提示包含智能体所需**所有**上下文——文件路径、URL、服务器地址和清晰的指令。
:::
...
对于共享的团队机器人，请使用 Docker 作为终端后端，这样智能体命令将在容器中运行，而不是在您的主机上：
