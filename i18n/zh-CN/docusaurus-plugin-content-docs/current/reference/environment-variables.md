...
| `COPILOT_GITHUB_TOKEN` | GitHub 令牌，用于 Copilot API — 第一优先级（OAuth `gho_*` 或细粒度 PAT `github_pat_*`；经典 PAT `ghp_*` **不支持**） |
...
| `DASHSCOPE_API_KEY` | Alibaba Cloud DashScope API 密钥，用于 Qwen 模型 ([modelstudio.console.alibabacloud.com](https://modelstudio.console.alibabacloud.com/)) |
...
| `NVIDIA_BASE_URL` | 覆盖 NVIDIA 基础 URL（默认：`https://integrate.api.nvidia.com/v1`；设置为 `http://localhost:8000/v1` 以使用本地 NIM 端点） |
...
| `AWS_REGION` | AWS 区域，用于 Bedrock 推理（例如 `us-east-1`、`eu-central-1`）。由 boto3 读取。 |
| `AWS_PROFILE` | AWS 命名配置文件，用于 Bedrock 身份验证（读取 `~/.aws/credentials`）。留空则使用默认的 boto3 凭据链。 |
| `BEDROCK_BASE_URL` | 覆盖 Bedrock 运行时基础 URL（默认：`https://bedrock-runtime.us-east-1.amazonaws.com`；通常留空并使用 `AWS_REGION` 代替） |
| `HERMES_QWEN_BASE_URL` | Qwen Portal 基础 URL 覆盖（默认：`https://portal.qwen.ai/v1`） |
| `OPENCODE_ZEN_API_KEY` | OpenCode Zen API 密钥 — 精选模型的按需访问 ([opencode.ai](https://opencode.ai/auth)) |
...
| `CLAUDE_CODE_OAUTH_TOKEN` | 如果您手动导出了一个 Claude Code 令牌，则显式覆盖 |
| `HERMES_MODEL` | 在进程级别覆盖模型名称（由 cron 调度器使用；正常情况建议使用 `config.yaml`） |
| `VOICE_TOOLS_OPENAI_KEY` | 首选 OpenAI 密钥，用于 OpenAI 语音转文本和文本转语音提供商 |
...
| `HERMES_HOME` | 覆盖 Hermes 配置目录（默认：`~/.hermes`）。还定义了网关 PID 文件和 systemd 服务名称的作用域，因此可以并发运行多个安装 |

## 提供商认证（OAuth）

对于原生的 Anthropic 认证，Hermes 优先选择 Claude Code 自身的凭据文件，因为它们可以自动刷新。环境变量如 `ANTHROPIC_TOKEN` 仍然有用，作为手动覆盖，但它们不再是 Claude Pro/Max 登录的首选方式。
...
| `HERMES_NOUS_MIN_KEY_TTL_SECONDS` | 重新生成前代理密钥的最小 TTL（默认：1800 = 30 分钟） |
| `HERMES_NOUS_TIMEOUT_SECONDS` | Nous 凭据/令牌流程的 HTTP 超时 |
| `HERMES_DUMP_REQUESTS` | 将 API 请求负载记录到日志文件中（`true`/`false`） |
| `HERMES_PREFILL_MESSAGES_FILE` | 注入到 API 调用时的临时预填充消息的 JSON 文件路径 |
...
| `GITHUB_TOKEN` | GitHub 令牌，用于技能中心（更高的 API 速率限制，发布技能） |
| `HONCHO_API_KEY` | 跨会话用户建模 ([honcho.dev](https://honcho.dev/)) |
| `HONCHO_BASE_URL` | 自托管 Honcho 实例的基础 URL（默认：Honcho 云）。本地实例不需要 API 密钥 |
| `SUPERMEMORY_API_KEY` | 语义长期记忆，带个人资料回忆和会话摄取 ([supermemory.ai](https://supermemory.ai)) |
...
这些变量配置 [工具网关](/docs/user-guide/features/tool-gateway)，适用于付费 Nous 订阅者或自托管网关部署。大多数用户不需要设置这些 — 网关通过 `hermes model` 或 `hermes tools` 自动配置。
...
| `TERMINAL_DOCKER_FORWARD_ENV` | 明确转发到 Docker 终端会话的环境变量名称 JSON 数组。注意：技能声明的 `required_environment_variables` 会自动转发 — 您只需要为未由任何技能声明的变量设置此项。 |
...
对于云沙盒后端，持久化是基于文件系统的。`TERMINAL_LIFETIME_SECONDS` 控制 Hermes 清理空闲终端会话的时间，稍后恢复可能会重建沙盒而不是保持相同的活动进程运行。
...
| `TELEGRAM_ALLOWED_USERS` | 允许使用该机器人的逗号分隔的用户 ID |
...
| `TELEGRAM_WEBHOOK_SECRET` | 验证更新来自 Telegram 的机密令牌 |
| `TELEGRAM_REACTIONS` | 在处理期间启用消息上的表情符号反应（默认：`false`） |
| `TELEGRAM_REPLY_TO_MODE` | 回复引用行为：`off`、`first`（默认）或 `all`。匹配 Discord 模式。 |
| `TELEGRAM_IGNORED_THREADS` | 机器人从不回复的逗号分隔的 Telegram 论坛主题/线程 ID |
| `TELEGRAM_PROXY` | Telegram 连接的代理 URL — 覆盖 `HTTPS_PROXY`。支持 `http://`、`https://`、`socks5://` |
| `DISCORD_BOT_TOKEN` | Discord 机器人令牌 |
| `DISCORD_ALLOWED_USERS` | 允许使用该机器人的逗号分隔的 Discord 用户 ID |
| `DISCORD_ALLOWED_ROLES` | 允许使用该机器人的逗号分隔的角色 ID（与 `DISCORD_ALLOWED_USERS` OR 操作）。自动启用成员意图。当管理团队变动时很有用 — 角色授予会自动传播。 |
| `DISCORD_ALLOWED_CHANNELS` | 逗号分隔的 Discord 频道 ID。设置时，机器人只在这些频道中响应（加上允许的私信）。覆盖 `config.yaml` 中的 `discord.allowed_channels`。 |
...
| `DISCORD_COMMAND_SYNC_POLICY` | Discord slash-command 启动同步策略：`safe`（差异并协调）、`bulk`（旧版 `tree.sync()`）或 `off` |
| `DISCORD_REQUIRE_MENTION` | 在服务器频道中响应前需要 @提及 |
| `DISCORD_FREE_RESPONSE_CHANNELS` | 机器人无需 @提及 即可响应的逗号分隔的频道 ID |
| `DISCORD_AUTO_THREAD` | 当支持时自动创建长回复的线程 |
| `DISCORD_REACTIONS` | 在处理期间启用消息上的表情符号反应（默认：`true`） |
| `DISCORD_IGNORED_CHANNELS` | 机器人从不回复的逗号分隔的频道 ID |
| `DISCORD_NO_THREAD_CHANNELS` | 机器人无需自动线程即可响应的逗号分隔的频道 ID |
...
| `DISCORD_ALLOW_MENTION_REPLIED_USER` | 回复用户消息时 ping 作者（默认：`true`）。 |
...
| `SMS_WEBHOOK_PORT` | 入站 SMS webhook 的监听端口（默认：`8080`) |
...
| `EMAIL_ALLOWED_USERS` | 允许向机器人发送消息的逗号分隔的邮箱地址 |
| `EMAIL_HOME_ADDRESS` | 主动邮件交付的默认收件人 |
| `EMAIL_HOME_ADDRESS_NAME` | email 目标目标的显示名称 |
| `EMAIL_POLL_INTERVAL` | email 轮询间隔（秒） |
| `EMAIL_ALLOW_ALL_USERS` | 允许所有入站 email 发送者 |
...
| `FEISHU_HOME_CHANNEL` | cron 交付和通知的飞书聊天 ID |
| `WECOM_BOT_ID` | WeCom AI Bot ID（管理控制台获取） |
| `WECOM_SECRET` | WeCom AI Bot 密钥 |
| `WECOM_WEBSOCKET_URL` | 自定义 WebSocket URL（默认：`wss://openws.work.weixin.qq.com`) |
| `WECOM_ALLOWED_USERS` | 允许向机器人发送消息的逗号分隔的 WeCom 用户 ID |
...
| `WEIXIN_CDN_BASE_URL` | 覆盖微信 CDN 基础 URL（媒体，默认：`https://novac2c.cdn.weixin.qq.com/c2c`) |
| `WEIXIN_DM_POLICY` | 私信政策：`open`、`allowlist`、`pairing`、`disabled`（默认：`open`) |
| `WEIXIN_GROUP_POLICY` | 群聊政策：`open`、`allowlist`、`disabled`（默认：`disabled`) |
| `WEIXIN_ALLOWED_USERS` | 允许向机器人私信的逗号分隔的微信用户 ID |
| `WEIXIN_GROUP_ALLOWED_USERS` | 允许与机器人交互的逗号分隔的微信群组 ID |
...
| `QQ_ALLOWED_USERS` | 允许向机器人发送消息的逗号分隔的 QQ 用户 openIDs |
| `QQ_GROUP_ALLOWED_USERS` | 群聊 @消息访问的逗号分隔的 QQ 群组 ID |
...
| `QQ_SANDBOX` | 将 QQ Bot 路由到沙盒网关进行开发测试 (`true`/`false`)。与 [q.qq.com](https://q.qq.com) 的沙盒应用凭据一起使用。 |
...
| `MATTERMOST_FREE_RESPONSE_CHANNELS` | 机器人无需 `@mention` 即可响应的逗号分隔的频道 ID |
...
| `MATRIX_DEVICE_ID` | Matrix E2EE 持久化的稳定设备 ID（例如 `HERMES_BOT`)。没有它，E2EE 密钥每次启动都会轮换，导致历史房间解密中断。 |
| `MATRIX_REACTIONS` | 在处理入站消息的生命周期中启用表情符号反应（默认：`true`)。设置为 `false` 以禁用。 |
| `MATRIX_REQUIRE_MENTION` | 在房间中需要 `@mention`（默认：`true`)。设置为 `false` 以响应所有消息。 |
| `MATRIX_FREE_RESPONSE_ROOMS` | 机器人无需 `@mention` 即可响应的逗号分隔的房间 ID |
| `MATRIX_AUTO_THREAD` | 为房间消息自动创建线程（默认：`true`) |
| `MATRIX_DM_MENTION_THREADS` | 在 DM 中 @提及 机器人时创建线程（默认：`false`) |
| `MATRIX_RECOVERY_KEY` | 设备密钥轮换后的跨签名验证恢复密钥。建议启用了跨签名的 E2EE 设置使用。 |
...
| `API_SERVER_KEY` | API 服务器认证 Bearer 令牌。非回环绑定时强制执行。 |
| `API_SERVER_CORS_ORIGINS` | 直接调用 API 服务器的逗号分隔的浏览器源（例如 `http://localhost:3000,http://127.0.0.1:3000`)。默认：禁用。 |
| `API_SERVER_PORT` | API 服务器端口（默认：`8642`) |
| `API_SERVER_HOST` | API 服务器的宿主/绑定地址（默认：`127.0.0.1`)。使用 `0.0.0.0` 进行网络访问 — 需要 `API_SERVER_KEY` 和窄的 `API_SERVER_CORS_ORIGINS` 白名单。 |
| `API_SERVER_MODEL_NAME` | `/v1/models` 上宣传的模型名称。默认为配置文件名称（或默认配置文件的 `hermes-agent`)。对于多用户设置很有用，前端如 Open WebUI 需要每个连接有 distinct 模型名称。 |
| `GATEWAY_PROXY_URL` | 转发消息到的远程 Hermes API 服务器 URL ([proxy mode](/docs/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos))。设置时，网关仅处理平台 I/O — 所有智能体工作委托给远程服务器。也可通过 `config.yaml` 中的 `gateway.proxy_url` 配置。 |
| `GATEWAY_PROXY_KEY` | proxy mode 中与远程 API 服务器认证的 Bearer 令牌。必须匹配远程主机上的 `API_SERVER_KEY`。 |
| `MESSAGING_CWD` | messaging mode 中终端命令的工作目录（默认：`~`) |
| `GATEWAY_ALLOWED_USERS` | 所有平台上允许的逗号分隔的用户 ID |
...
| `HERMES_TOOL_PROGRESS` | 已弃用的兼容性变量，用于工具进度显示。优先使用 `config.yaml` 中的 `display.tool_progress`。 |
| `HERMES_TOOL_PROGRESS_MODE` | 已弃用的兼容性变量，用于工具进度模式。优先使用 `config.yaml` 中的 `display.tool_progress`。 |
...
| `HERMES_API_CALL_STALE_TIMEOUT` | 非流式陈旧调用超时（秒）（默认：`300`)。为本地提供商自动禁用。也可通过 `config.yaml` 中的 `providers.<id>.stale_timeout_seconds` 或 `providers.<id>.models.<model>.stale_timeout_seconds` 配置。 |
| `HERMES_STREAM_READ_TIMEOUT` | 流式套接字读取超时（秒）（默认：`120`)。为本地提供商自动增加到 `HERMES_API_TIMEOUT`。如果本地 LLM 在长时间代码生成期间超时，请增加此值。 |
| `HERMES_STREAM_STALE_TIMEOUT` | 陈旧流检测超时（秒）（默认：`180`)。为本地提供商自动禁用。如果在此窗口内没有块到达，则触发连接终止。 |
| `HERMES_EXEC_ASK` | 在 gateway mode 中启用执行批准提示 (`true`/`false`) |
| `HERMES_ENABLE_PROJECT_PLUGINS` | 启用从 `./.hermes/plugins/` 自动发现仓库本地插件 (`true`/`false`，默认：`false`) |
| `HERMES_BACKGROUND_NOTIFICATIONS` | gateway 中的后台进程通知模式：`all`（默认）、`result`、`error`、`off` |
| `HERMES_EPHEMERAL_SYSTEM_PROMPT` | 注入到 API 调用时的临时系统提示（从不持久化到会话） |
| `DELEGATION_MAX_CONCURRENT_CHILDREN` | `delegate_task` 批处理中最大并行子智能体数量（默认：`3`，1 的下限，无上界）。也可通过 `config.yaml` 中的 `delegation.max_concurrent_children` 配置 — 配置值优先。 |
...
| `HERMES_TUI_DIR` | 预构建的 `ui-tui/` 目录路径（必须包含 `dist/entry.js` 和 populated `node_modules`)。由发行版和 Nix 用于跳过首次启动的 `npm install`。 |

## Cron 调度器

| 变量 | 描述 |
|----------|-------------|
| `HERMES_CRON_TIMEOUT` | cron 作业智能体运行的不活动时间（秒）（默认：`600`)。智能体在 actively 调用工具或接收流令牌时可以无限期运行 — 这只会在空闲时触发。设置为 `0` 表示无限制。 |
| `HERMES_CRON_SCRIPT_TIMEOUT` | 附加到 cron 作业的预运行脚本超时（秒）（默认：`120`)。为需要更长执行时间的脚本覆盖（例如，用于反机器人 timing 的随机延迟）。也可通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |
...
上下文压缩仅通过 `config.yaml` 配置 — 没有相关环境变量。阈值设置在 `compression:` 块中，摘要模型/提供商位于 `auxiliary.compression:` 下。
...
compression:
  ...
  protect_last_n: 20         # 最小最近消息数，保持未压缩
```

:::info 旧版本迁移
加载时，带有 `compression.summary_model`、`compression.summary_provider` 和 `compression.summary_base_url` 的旧配置会自动迁移到 `auxiliary.compression.*`。
:::
...
对于特定任务的直接端点，Hermes 使用任务的配置 API 密钥或 `OPENAI_API_KEY`。它不会为那些自定义端点重用 `OPENROUTER_API_KEY`。
...
这些放在 `~/.hermes/config.yaml` 的 `provider_routing` 部分：
