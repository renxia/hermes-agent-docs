...
所有变量均位于 `~/.hermes/.env` 文件中。你也可以使用 `hermes config set VAR value` 命令设置它们。
...
| `NOUS_BASE_URL` | 覆盖 Nous Portal 基础 URL（很少需要；仅限开发/测试用途） |
...
| `OPENAI_API_KEY` | 自定义兼容 OpenAI 的端点的 API 密钥（与 `OPENAI_BASE_URL` 配合使用） |
...
| `AZURE_ANTHROPIC_KEY` | Azure Anthropic API 密钥，用于 `provider: anthropic` + `base_url` 指向 Azure Foundry Claude 部署（当 Anthropic 和 Azure Anthropic 同时配置时，可替代 `ANTHROPIC_API_KEY`） |
...
| `CLAUDE_CODE_OAUTH_TOKEN` | 如果你手动导出了一个显式的 Claude Code 令牌，则覆盖此值 |
...
| `HERMES_HOME` | 覆盖 Hermes 配置目录（默认值：`~/.hermes`）。同时限定网关 PID 文件和 systemd 服务名称的作用域，因此多个安装实例可以并发运行 |

## 提供者身份验证（OAuth）

对于原生的 Anthropic 身份验证，Hermes 更倾向于使用 Claude Code 自身的凭据文件（如果存在），因为这些凭据可以自动刷新。诸如 `ANTHROPIC_TOKEN` 之类的环境变量仍可作为手动覆盖项使用，但它们不再是登录 Claude Pro/Max 的首选方式。

| 变量 | 描述 |
|----------|-------------|
| `HERMES_INFERENCE_PROVIDER` | 覆盖提供者选择：`auto`、`custom`、`openrouter`、`nous`、`openai-codex`、`copilot`、`copilot-acp`、`anthropic`、`huggingface`、`gemini`、`zai`、`kimi-coding`、`kimi-coding-cn`、`minimax`、`minimax-cn`、`kilocode`、`xiaomi`、`arcee`、`gmi`、`alibaba`、`deepseek`、`nvidia`、`ollama-cloud`、`xai`（别名 `grok`）、`google-gemini-cli`、`qwen-oauth`、`bedrock`、`opencode-zen`、`opencode-go`、`ai-gateway`（默认值：`auto`） |
| `HERMES_PORTAL_BASE_URL` | 覆盖 Nous 门户 URL（用于开发/测试） |
| `NOUS_INFERENCE_BASE_URL` | 覆盖 Nous 推理 API URL |
| `HERMES_NOUS_MIN_KEY_TTL_SECONDS` | 重新生成前智能体密钥的最小 TTL（默认值：1800 = 30 分钟） |
...
| `SUPERMEMORY_API_KEY` | 具有配置文件回忆和会话摄取功能的语义长期记忆（[supermemory.ai](https://supermemory.ai)） |
...
用于捆绑的 [`observability/langfuse`](/docs/user-guide/features/built-in-plugins#observabilitylangfuse) 插件的环境变量。使用 `hermes tools → Langfuse Observability` 或在 `~/.hermes/.env` 中手动设置这些变量。还必须启用该插件（`hermes plugins enable observability/langfuse`），这些设置才会生效。

| 变量 | 描述 |
|----------|-------------|
| `HERMES_LANGFUSE_PUBLIC_KEY` | Langfuse 项目公钥（`pk-lf-...`）。必需。 |
| `HERMES_LANGFUSE_SECRET_KEY` | Langfuse 项目私钥（`sk-lf-...`）。必需。 |
| `HERMES_LANGFUSE_BASE_URL` | Langfuse 服务器 URL（默认值：`https://cloud.langfuse.com`）。自托管时设置。 |
| `HERMES_LANGFUSE_ENV` | 跟踪上的环境标签（`production`、`staging`……） |
| `HERMES_LANGFUSE_RELEASE` | 跟踪上的发布/版本标签 |
...
| `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY` / `LANGFUSE_BASE_URL` | 标准 Langfuse SDK 名称。当 `HERMES_LANGFUSE_*` 等效项未设置时，接受作为备用项。 |
...
| 变量 | 描述 |
|----------|-------------|
| `TOOL_GATEWAY_DOMAIN` | 工具网关路由的基础域（默认值：`nousresearch.com`） |
| `TOOL_GATEWAY_SCHEME` | 网关 URL 的 HTTP 或 HTTPS 方案（默认值：`https`） |
| `TOOL_GATEWAY_USER_TOKEN` | 工具网关的身份验证令牌（通常由 Nous 身份验证自动填充） |
...
| `TERMINAL_DOCKER_FORWARD_ENV` | 需要显式转发到 Docker 终端会话的环境变量名称的 JSON 数组。注意：由技能声明的 `required_environment_variables` 会自动转发 — 您只需在此处添加未被任何技能声明的变量。 |
| `TERMINAL_DOCKER_VOLUMES` | 额外的 Docker 卷挂载（以逗号分隔的 `主机路径:容器路径` 对） |
| `TERMINAL_DOCKER_MOUNT_CWD_TO_WORKSPACE` | 高级可选功能：将启动时的当前工作目录挂载到 Docker 的 `/workspace` 目录（`true`/`false`，默认值：`false`） |
...
| `SUDO_PASSWORD` | 启用无需交互提示的 sudo 权限 |

对于云沙箱后端，持久性是基于文件系统的。`TERMINAL_LIFETIME_SECONDS` 控制 Hermes 清理空闲终端会话的时间，后续恢复时可能会重新创建沙箱，而不是保持相同的活动进程运行。
...
| `TERMINAL_SSH_PERSISTENT` | 覆盖 SSH 的持久化 shell 设置（默认值：遵循 `TERMINAL_PERSISTENT_SHELL`） |
...
| `TERMINAL_CONTAINER_MEMORY` | 内存大小（MB）（默认值：5120） |
| `TERMINAL_CONTAINER_DISK` | 磁盘大小（MB）（默认值：51200） |
| `TERMINAL_CONTAINER_PERSISTENT` | 跨会话持久化容器文件系统（默认值：`true`） |
| `TERMINAL_SANDBOX_DIR` | 工作区和覆盖层的主机目录（默认值：`~/.hermes/sandboxes/`） |
...
| `TELEGRAM_WEBHOOK_SECRET` | 用于验证更新是否来自 Telegram 的密钥 |
| `TELEGRAM_REACTIONS` | 在处理期间启用消息上的表情符号反应（默认值：`false`） |
...
| `DISCORD_ALLOWED_ROLES` | 允许使用机器人的 Discord 角色 ID（逗号分隔）（与 `DISCORD_ALLOWED_USERS` 取并集）。自动启用“成员”意图。适用于管理团队变动频繁的场景 — 角色权限会自动传播。 |
| `DISCORD_ALLOWED_CHANNELS` | Discord 频道 ID（逗号分隔）。设置后，机器人仅在这些频道中响应（如果允许，还包括私信）。覆盖 `config.yaml` 中的 `discord.allowed_channels`。 |
...
| `DISCORD_COMMAND_SYNC_POLICY` | Discord 斜杠命令启动同步策略：`safe`（差异比对并协调）、`bulk`（旧版 `tree.sync()`）或 `off` |
| `DISCORD_REQUIRE_MENTION` | 在服务器频道中响应前需要 @提及 |
| `DISCORD_FREE_RESPONSE_CHANNELS` | 无需提及即可响应的频道 ID（逗号分隔） |
| `DISCORD_AUTO_THREAD` | 支持时自动创建长回复的线程 |
| `DISCORD_REACTIONS` | 在处理期间启用消息上的表情符号反应（默认值：`true`） |
...
| `WHATSAPP_MODE` | `bot`（独立号码）或 `self-chat`（向自己发消息） |
| `WHATSAPP_ALLOWED_USERS` | 允许的电话号码（带国家代码，不带 `+`）（逗号分隔），或 `*` 允许所有发送者 |
| `WHATSAPP_ALLOW_ALL_USERS` | 无需白名单即允许所有 WhatsApp 发送者（`true`/`false`） |
...
| `SIGNAL_ALLOW_ALL_USERS` | 无需白名单即允许所有 Signal 用户 |
...
| `SMS_ALLOW_ALL_USERS` | 无需白名单即允许所有 SMS 发送者 |
...
| `EMAIL_ALLOWED_USERS` | 允许向机器人发送消息的电子邮件地址（逗号分隔） |
...
| `DINGTALK_ALLOWED_USERS` | 允许向机器人发送消息的钉钉用户 ID（逗号分隔） |
...
| `FEISHU_ALLOWED_USERS` | 允许向机器人发送消息的飞书用户 ID（逗号分隔） |
...
| `WECOM_ALLOWED_USERS` | 允许向机器人发送消息的企业微信用户 ID（逗号分隔） |
...
| `WEIXIN_ALLOWED_USERS` | 允许向机器人发送私信的微信用户 ID（逗号分隔） |
| `WEIXIN_GROUP_ALLOWED_USERS` | 允许与机器人交互的微信群组 ID（逗号分隔） |
| `WEIXIN_HOME_CHANNEL` | 定时任务投递和通知的微信聊天 ID |
| `WEIXIN_HOME_CHANNEL_NAME` | 微信主频道的显示名称 |
| `WEIXIN_ALLOW_ALL_USERS` | 无需白名单即允许所有微信用户（`true`/`false`） |
...
| `QQ_ALLOWED_USERS` | 允许向机器人发送消息的 QQ 用户 openID（逗号分隔） |
| `QQ_GROUP_ALLOWED_USERS` | 允许群组 @消息访问的 QQ 群组 ID（逗号分隔） |
...
| `MATRIX_ALLOWED_USERS` | 允许向机器人发送消息的 Matrix 用户 ID（例如 `@alice:matrix.org`）（逗号分隔） |
| `MATRIX_HOME_ROOM` | 主动消息投递的房间 ID（例如 `!abc123:matrix.org`） |
| `MATRIX_ENCRYPTION` | 启用端到端加密（`true`/`false`，默认值：`false`） |
| `MATRIX_DEVICE_ID` | 用于 E2EE 持久化的稳定 Matrix 设备 ID（例如 `HERMES_BOT`）。若无此设置，E2EE 密钥将在每次启动时轮换，导致历史房间解密失败。 |
| `MATRIX_REACTIONS` | 在入站消息上启用处理生命周期表情符号反应（默认值：`true`）。设为 `false` 可禁用。 |
| `MATRIX_REQUIRE_MENTION` | 在房间中需要 `@mention`（默认值：`true`）。设为 `false` 可响应所有消息。 |
| `MATRIX_FREE_RESPONSE_ROOMS` | 机器人无需 `@mention` 即可响应的房间 ID（逗号分隔） |
| `MATRIX_AUTO_THREAD` | 为房间消息自动创建线程（默认值：`true`） |
| `MATRIX_DM_MENTION_THREADS` | 在私信中被 `@mentioned` 时创建线程（默认值：`false`） |
| `MATRIX_RECOVERY_KEY` | 设备密钥轮换后用于交叉签名验证的恢复密钥。建议在使用交叉签名功能的 E2EE 设置中使用。 |
...
| `WEBHOOK_SECRET` | 用于 Webhook 签名验证的全局 HMAC 密钥（当路由未指定自身密钥时作为备用） |
| `API_SERVER_ENABLED` | 启用 OpenAI 兼容 API 服务器（`true`/`false`）。与其他平台并行运行。 |
| `API_SERVER_KEY` | API 服务器身份验证的 Bearer 令牌。对非环回绑定强制执行。 |
| `API_SERVER_CORS_ORIGINS` | 允许直接调用 API 服务器的浏览器源（逗号分隔）（例如 `http://localhost:3000,http://127.0.0.1:3000`）。默认值：禁用。 |
| `API_SERVER_PORT` | API 服务器端口（默认值：`8642`） |
| `API_SERVER_HOST` | API 服务器主机/绑定地址（默认值：`127.0.0.1`）。使用 `0.0.0.0` 可启用网络访问 — 需配合 `API_SERVER_KEY` 和严格的 `API_SERVER_CORS_ORIGINS` 白名单。 |
| `API_SERVER_MODEL_NAME` | `/v1/models` 上公布的模型名称。默认为配置文件名称（或默认配置文件的 `hermes-agent`）。适用于多用户设置，前端（如 Open WebUI）需要每个连接具有不同的模型名称。 |
...
| `GATEWAY_ALLOWED_USERS` | 所有平台允许的用户 ID（逗号分隔） |
| `GATEWAY_ALLOW_ALL_USERS` | 无需白名单即允许所有用户（`true`/`false`，默认值：`false`） |
...
| `HERMES_TOOL_PROGRESS` | 用于工具进度显示的不推荐兼容性变量。请优先使用 `config.yaml` 中的 `display.tool_progress`。 |
| `HERMES_TOOL_PROGRESS_MODE` | 用于工具进度模式的不推荐兼容性变量。请优先使用 `config.yaml` 中的 `display.tool_progress`。 |
...
| `HERMES_API_CALL_STALE_TIMEOUT` | 非流式调用陈旧调用超时时间（秒）（默认值：`300`）。本地提供程序在未设置时自动禁用。也可通过 `config.yaml` 中的 `providers.<id>.stale_timeout_seconds` 或 `providers.<id>.models.<model>.stale_timeout_seconds` 配置。 |
| `HERMES_STREAM_READ_TIMEOUT` | 流式套接字读取超时时间（秒）（默认值：`120`）。对于本地提供程序，自动增加至 `HERMES_API_TIMEOUT`。如果本地 LLM 在生成长代码时超时，请增加此值。 |
| `HERMES_STREAM_STALE_TIMEOUT` | 陈旧流检测超时时间（秒）（默认值：`180`）。本地提供程序自动禁用。如果在此窗口内未收到任何数据块，则触发连接终止。 |
...
| `DELEGATION_MAX_CONCURRENT_CHILDREN` | 每个 `delegate_task` 批处理的最大并行子智能体数量（默认值：`3`，最小值为 1，无上限）。也可通过 `config.yaml` 中的 `delegation.max_concurrent_children` 配置 —— 配置文件中的值优先。 |

## 界面

| 变量 | 说明 |
|----------|-------------|
| `HERMES_TUI` | 设置为 `1` 时启动 [TUI](../user-guide/tui.md) 而不是经典 CLI。等效于传递 `--tui`。 |
| `HERMES_TUI_DIR` | 预构建的 `ui-tui/` 目录路径（必须包含 `dist/entry.js` 和已填充的 `node_modules`）。发行版和 Nix 使用此目录跳过首次启动时的 `npm install`。 |

## 定时任务调度器

| 变量 | 说明 |
|----------|-------------|
| `HERMES_CRON_TIMEOUT` | 定时任务智能体运行的不活动超时时间（秒）（默认值：`600`）。智能体在主动调用工具或接收流令牌时可以无限期运行 —— 此设置仅在空闲时触发。设置为 `0` 表示无限制。 |
| `HERMES_CRON_SCRIPT_TIMEOUT` | 附加到定时任务的预运行脚本的超时时间（秒）（默认值：`120`）。适用于需要更长执行时间的脚本（例如，用于反机器人计时的随机延迟）。也可通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |
...
上下文压缩仅通过 `config.yaml` 配置 —— 没有对应的环境变量。阈值设置位于 `compression:` 块中，而摘要模型/提供程序位于 `auxiliary.compression:` 下。
...
compression:
  ...
  protect_last_n: 20         # 保持未压缩状态的最小最近消息数
```
...
| 变量 | 说明 |
|----------|-------------|
| `AUXILIARY_VISION_PROVIDER` | 覆盖视觉任务的提供程序 |
...
| `AUXILIARY_WEB_EXTRACT_PROVIDER` | 覆盖网页提取/摘要的提供程序 |
...
主模型回退仅通过 `config.yaml` 配置 —— 没有对应的环境变量。添加一个包含 `provider` 和 `model` 键的 `fallback_model` 部分，以便在主模型遇到错误时启用自动故障转移。
...
详见 [回退提供程序](/docs/user-guide/features/fallback-providers)。

## 提供程序路由（仅限 config.yaml）

这些配置项位于 `~/.hermes/config.yaml` 的 `provider_routing` 部分：

| 键 | 说明 |
|-----|-------------|
| `sort` | 排序提供程序：`"price"`（默认值）、`"throughput"` 或 `"latency"` |
| `only` | 允许的提供程序 slug 列表（例如：`["anthropic", "google"]`） |
| `ignore` | 跳过的提供程序 slug 列表 |
| `order` | 按顺序尝试的提供程序 slug 列表 |
| `require_parameters` | 仅使用支持所有请求参数的提供程序（`true`/`false`） |
| `data_collection` | `"allow"`（默认值）或 `"deny"` 以排除数据存储提供程序 |

:::tip
使用 `hermes config set` 设置环境变量 —— 它会自动将其保存到正确的文件（`.env` 用于机密，`config.yaml` 用于其他所有内容）。
:::