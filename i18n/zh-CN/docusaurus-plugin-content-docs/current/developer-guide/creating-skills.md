...
技能是为 Hermes Agent 添加新功能的首选方式。它们比工具更容易创建，不需要修改代理代码，并且可以与社区共享。

## 技能还是工具？

当满足以下条件时，应将其作为**技能 (Skill)**：
- 该能力可以表示为指令 + Shell 命令 + 现有工具的组合
- 它封装了一个外部 CLI 或 API，代理可以通过 `terminal` 或 `web_extract` 调用
- 它不需要在代理中内置自定义 Python 集成或 API 密钥管理
...
3. **运行时注入：** 当技能加载时，其配置值会被解析并附加到技能消息中：
   ...
   ```
   代理无需读取 `config.yaml` 本身，就能看到配置的值。
