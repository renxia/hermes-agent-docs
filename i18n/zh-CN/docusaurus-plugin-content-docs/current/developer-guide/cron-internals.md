...
任务还可以通过 `script` 字段附加一个 Python 脚本。该脚本在每次智能体回合*之前*运行，其标准输出 (stdout) 作为上下文注入到提示中。这使得数据收集和变更检测模式成为可能：
...
这模仿了网关的行为——如果没有它，cron 智能体在遇到速率限制时将无法尝试恢复而直接失败。
