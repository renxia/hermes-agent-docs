...
```
  ...
  ─────────────────► │   (预代理，粗估) │  大型会话的安全网
                     └─────────────┬────────────┘
                                   ...
                                   ▼
                     ┌──────────────────────────┐
                     │   代理 ContextCompressor │  在上下文达到 50% 时触发 (默认)
...
在代理处理消息之前运行。它防止了在回合之间（例如，Telegram/Discord 的过夜积累）
会话增长过大时发生的 API 失败。
...
网关卫生阈值故意设置得高于代理的压缩器。
将其设置为 50%（与代理相同）会导致在长网关会话的每个回合都过早压缩。

### 2. 代理 ContextCompressor (50% 阈值，可配置)

位于 `agent/context_compressor.py`。这是**主要的压缩系统**，它在代理的工具循环内运行，并可以访问准确的、
API 报告的 token 计数。
...
:::warning 摘要模型上下文长度
摘要模型必须具有**至少与主代理模型相同大小**的上下文窗口。整个中间部分在一个 `call_llm(task="compression")` 调用中发送给摘要模型。如果摘要模型的上下文较小，API 将返回上下文长度错误 — `_generate_summary()` 会捕获此错误，记录警告，并返回 `None`。压缩器随后会**不带摘要**地删除中间回合，从而静默地丢失对话上下文。这是导致压缩质量下降的最常见原因。
:::
