...
sidebar_position: 1
title: "架构"
description: "Hermes 智能体内部结构 —— 主要子系统、执行路径、数据流以及后续阅读指引"
---

# 架构

本页面是 Hermes 智能体内部结构的顶层地图。使用它来定位自己在代码库中的位置，然后深入阅读特定子系统的文档以了解实现细节。
...
│   ├── builtin_hooks/        # 始终注册的钩子扩展点（未随附任何钩子）
...
5. **[添加提供器](./adding-providers.md)** — 添加新提供器的实用指南
...
同步编排引擎（`run_agent.py` 中的 `AIAgent`）。处理提供器选择、提示构建、工具执行、重试、回退、回调、压缩和持久化。支持三种 API 模式，适配不同的提供器后端。
...
- **`prompt_builder.py`** — 从以下内容组装系统提示：人格（SOUL.md）、记忆（MEMORY.md、USER.md）、技能、上下文文件（AGENTS.md、.hermes.md）、工具使用指南以及模型特定指令
...
由 CLI、网关、定时任务、ACP 和辅助调用共享的运行时解析器。将 `(提供器, 模型)` 元组映射为 `(api_mode, api_key, base_url)`。支持 18+ 个提供器、OAuth 流程、凭据池和别名解析。

→ [提供器运行时解析](./provider-runtime.md)

### 工具系统

中央工具注册表（`tools/registry.py`），包含 52 个工具集中的 61 个已注册工具。每个工具文件在导入时自动注册。注册表负责模式收集、调度、可用性检查和错误包装。终端工具支持 7 种后端（本地、Docker、SSH、Daytona、Modal、Singularity、Vercel Sandbox）。

→ [工具运行时](./tools-runtime.md)

### 会话持久化

基于 SQLite 的会话存储，支持 FTS5 全文搜索。会话具有谱系跟踪（跨压缩的父/子关系）、按平台隔离以及带争用处理的原子写入。

→ [会话存储](./session-storage.md)

### 消息网关

长期运行进程，包含 20 个平台适配器、统一会话路由、用户授权（白名单 + DM 配对）、斜杠命令调度、钩子系统、定时任务滴答和后台维护。

→ [网关内部实现](./gateway-internals.md)

### 插件系统

三个发现源：`~/.hermes/plugins/`（用户）、`.hermes/plugins/`（项目）和 pip 入口点。插件通过上下文 API 注册工具、钩子和 CLI 命令。存在两种特殊插件类型：记忆提供器（`plugins/memory/`）和上下文引擎（`plugins/context_engine/`）。两者均为单选 — 每次只能激活一个，通过 `hermes plugins` 或 `config.yaml` 配置。

→ [插件指南](/docs/guides/build-a-hermes-plugin)，[记忆提供器插件](./memory-provider-plugin.md)

### 定时任务

一等智能体任务（非 Shell 任务）。任务以 JSON 格式存储，支持多种调度格式，可附加技能和脚本，并可投递至任何平台。

→ [定时任务内部实现](./cron-internals.md)
...
| 原则 | 实际含义 |
|---|---|
| **提示稳定性** | 系统提示在对话过程中不会改变。除非用户明确执行操作（`/model`），否则不会发生破坏缓存的变更。 |
| **可观察执行** | 每个工具调用都通过回调对用户可见。在命令行界面（CLI）中显示进度更新（旋转图标），在网关中显示聊天消息。 |
| **可中断** | API调用和工具执行可以由用户输入或信号在运行过程中取消。 |
| **平台无关核心** | 一个AIAgent类服务于CLI、网关、ACP、批处理和API服务器。平台差异体现在入口点，而非智能体本身。 |
| **松耦合** | 可选子系统（MCP、插件、记忆提供者、强化学习环境）采用注册模式和check_fn门控，而非硬依赖。 |
| **配置文件隔离** | 每个配置文件（`hermes -p <名称>`）拥有独立的HERMES_HOME、配置、记忆、会话和网关PID。多个配置文件可同时运行。 |
...
此依赖链意味着工具注册发生在导入时，在任何智能体实例创建之前。任何包含顶层`registry.register()`调用的`tools/*.py`文件都会被自动发现 — 无需手动维护导入列表。