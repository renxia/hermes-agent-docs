...
sidebar_position: 3
title: "Nix & NixOS 配置"
description: "使用 Nix 安装并部署 Hermes 智能体 — 从快速 `nix run` 到完全声明式的 NixOS 模块（支持容器模式）"
---

# Nix & NixOS 配置

Hermes 智能体提供了一个 Nix flake，包含三个集成层级：
...
:::info 与标准安装的区别
`curl | bash` 安装脚本会自行管理 Python、Node 及其依赖。Nix flake 则完全替代了这些 — 每个 Python 依赖项都是一个由 [uv2nix](https://github.com/pyproject-nix/uv2nix) 构建的 Nix 推导，运行时工具（Node.js、git、ripgrep、ffmpeg）被打包进二进制文件的 PATH 中。运行时没有 pip，没有 venv 激活，也没有 `npm install`。

**对于非 NixOS 用户**，这仅改变了安装步骤。后续所有操作（`hermes setup`、`hermes gateway install`、配置文件编辑）与标准安装完全相同。

**对于 NixOS 模块用户**，整个生命周期都不同：配置位于 `configuration.nix` 中，密钥通过 sops-nix/agenix 管理，服务是一个 systemd 单元，且 CLI 配置命令被禁用。您管理 hermes 的方式与您管理任何其他 NixOS 服务的方式相同。
:::

## 先决条件

- **启用了 flakes 的 Nix** — 推荐使用 [Determinate Nix](https://install.determinate.systems)（默认启用 flakes）
- 您要使用的服务的 **API 密钥**（至少需要一个 OpenRouter 或 Anthropic 密钥）
---
...
执行 `nix profile install` 后，`hermes`、`hermes-agent` 和 `hermes-acp` 将被添加到您的 PATH 中。从此处开始，工作流程与[标准安装](./installation.md)完全相同 —— `hermes setup` 会引导您选择提供商，`hermes gateway install` 会设置一个 launchd（macOS）或 systemd 用户服务，配置文件位于 `~/.hermes/`。
...
:::note
此模块需要 NixOS。对于非 NixOS 系统（macOS、其他 Linux 发行版），请使用 `nix profile install` 和上述标准 CLI 工作流程。
:::
...
就是这样。`nixos-rebuild switch` 会创建 `hermes` 用户，生成 `config.yaml`，连接密钥，并启动网关 —— 一个长期运行的服务，它将智能体连接到消息平台（Telegram、Discord 等）并监听传入的消息。

:::warning 密钥是必需的
上述 `environmentFiles` 行假定您已配置 [sops-nix](https://github.com/Mic92/sops-nix) 或 [agenix](https://github.com/ryantm/agenix)。该文件应至少包含一个 LLM 提供商密钥（例如 `OPENROUTER_API_KEY=sk-or-...`）。请参阅[密钥管理](#secrets-management)以获取完整设置。如果您还没有密钥管理器，可以使用普通文件作为起点 —— 只需确保它不是全局可读的：
...
设置 `addToSystemPackages = true` 会做两件事：将 `hermes` CLI 添加到您的系统 PATH **并** 设置系统范围的 `HERMES_HOME`，以便交互式 CLI 与网关服务共享状态（会话、技能、定时任务）。如果没有它，在 shell 中运行 `hermes` 会创建一个单独的 `~/.hermes/` 目录。
:::

:::info 容器感知 CLI
当 `container.enable = true` 且 `addToSystemPackages = true` 时，主机上的**每个** `hermes` 命令都会自动路由到受管理的容器中。这意味着您的交互式 CLI 会话在与网关服务相同的环境中运行 —— 可以访问所有容器安装的软件包和工具。

- 路由是透明的：`hermes chat`、`hermes sessions list`、`hermes version` 等都会在底层 exec 到容器中
- 所有 CLI 标志都会原样转发
- 如果容器未运行，CLI 会短暂重试（交互式使用时有 5 秒带旋转指示器，脚本中静默 10 秒），然后以清晰的错误失败 —— 不会静默回退
- 对于开发 hermes 代码库的开发人员，请设置 `HERMES_DEV=1` 以绕过容器路由并直接运行本地检出
...
列在 `hostUsers` 中的用户会自动添加到 `hermes` 组以获得文件权限访问。

**Podman 用户：** NixOS 服务以 root 身份运行容器。Docker 用户通过 `docker` 组套接字获得访问权限，但 Podman 的 rootful 容器需要 sudo。为您的容器运行时授予无密码 sudo：
...
# 如果 addToSystemPackages 为 true，请测试 CLI
hermes version
...
| | **原生**（默认） | **容器** |
|---|---|---|
| 运行方式 | 主机上的强化 systemd 服务 | 持久性 Ubuntu 容器，绑定挂载 `/nix/store` |
| 安全性 | `NoNewPrivileges`、`ProtectSystem=strict`、`PrivateTmp` | 容器隔离，以非特权用户身份在内部运行 |
| 智能体是否可以自行安装软件包 | 否 —— 仅可使用 Nix 提供的 PATH 上的工具 | 是 —— `apt`、`pip`、`npm` 安装会在重启后保留 |
| 配置表面 | 相同 | 相同 |
...
`settings` 选项接受任意属性集，该属性集将呈现为 `config.yaml`。它支持跨多个模块定义进行深度合并（通过 `lib.recursiveUpdate`），因此您可以将配置拆分到多个文件中：
...
两者在求值时都会进行深度合并。Nix 声明的键始终优先于磁盘上现有 `config.yaml` 中的键，但**用户添加的 Nix 未触及的键会被保留**。这意味着如果智能体或手动编辑添加了诸如 `skills.disabled` 或 `streaming.enabled` 之类的键，它们会在 `nixos-rebuild switch` 后保留。

:::note 模型命名
`settings.model.default` 使用您的提供商期望的模型标识符。使用 [OpenRouter](https://openrouter.ai)（默认）时，这些看起来像 `"anthropic/claude-sonnet-4"` 或 `"google/gemini-3-flash"`。如果您直接使用提供商（Anthropic、OpenAI），请设置 `settings.model.base_url` 以指向其 API 并使用其原生模型 ID（例如 `"claude-sonnet-4-20250514"`）。当未设置 `base_url` 时，Hermes 默认为 OpenRouter。
:::
...
这会完全绕过 `settings` —— 不合并，不生成。该文件在每次激活时都会原样复制到 `$HERMES_HOME/config.yaml`。
...
| 使额外工具对智能体可用 | `extraPackages` | `[ pkgs.pandoc pkgs.imagemagick ]` |
...
Nix 表达式中的值最终会存储在 `/nix/store` 中，该目录对所有用户可读。请始终使用带有密钥管理器的 `environmentFiles`。
:::
...
仅当 `auth.json` 尚不存在时才会复制该文件（除非设置了 `authFileForceOverwrite = true`）。运行时的 OAuth 令牌刷新会写入状态目录，并在重建过程中保留。

---

## 文档

`documents` 选项会将文件安装到智能体的工作目录中（即 `workingDirectory`，智能体将其读取为自己的工作空间）。Hermes 按惯例查找特定文件名：

- **`USER.md`** — 关于智能体正在交互的用户的上下文信息。
- 您在此处放置的任何其他文件对智能体而言均可见，作为工作空间文件。

智能体身份文件是独立的：Hermes 从其主 `SOUL.md` 文件加载，该文件位于 `$HERMES_HOME/SOUL.md`，在 NixOS 模块中对应 `${services.hermes-agent.stateDir}/.hermes/SOUL.md`。将 `SOUL.md` 放入 `documents` 只会创建一个工作空间文件，并不会替换主要的角色文件。
...
```nix
{
  services.hermes-agent.mcpServers = {
    ...
    github = {
      ...
      env.GITHUB_PERSONAL_ACCESS_TOKEN = "\${GITHUB_TOKEN}"; # 运行时从 .env 解析
    };
  };
}
```
...
:::提示
`env` 值中的环境变量在运行时会从 `$HERMES_HOME/.env` 解析。使用 `environmentFiles` 注入密钥 —— 切勿将令牌直接写入 Nix 配置。
:::
...
当 hermes 通过 NixOS 模块运行时，以下 CLI 命令将被**阻止**，并显示指向 `configuration.nix` 的描述性错误：
...
:::信息
如果您使用 `container.enable = true`，本节内容才相关。原生模式部署请跳过。
:::

启用容器模式时，hermes 运行在一个持久的 Ubuntu 容器内，Nix 构建的二进制文件以只读方式从主机绑定挂载：
...
Nix 构建的二进制文件之所以能在 Ubuntu 容器内运行，是因为 `/nix/store` 被绑定挂载 —— 它自带解释器和所有依赖项，因此不依赖容器的系统库。容器入口点通过 `current-package` 符号链接解析：`/data/current-package/bin/hermes gateway run --replace`。执行 `nixos-rebuild switch` 时，仅更新符号链接 —— 容器继续运行。
...
| `nix-collect-garbage` | 否（GC 根） | 持久保留 | 持久保留 | 持久保留 |
| 镜像变更（`container.image`） | **是** | 持久保留 | 持久保留 | **丢失** |
| 卷/选项变更 | **是** | 持久保留 | 持久保留 | **丢失** |
| `environment`/`environmentFiles` 变更 | 否 | 持久保留 | 持久保留 | 持久保留 |

仅当容器的**身份哈希**发生变化时，容器才会重建。哈希涵盖：schema 版本、镜像、`extraVolumes`、`extraOptions` 和入口点脚本。环境变量、设置、文档或 hermes 包本身的变更**不会**触发重建。

:::警告 可写层丢失
当身份哈希发生变化时（镜像升级、新增卷、新增容器选项），容器将被销毁并从 `container.image` 的全新拉取中重建。可写层中任何通过 `apt install`、`pip install` 或 `npm install` 安装的软件包都会丢失。`/data` 和 `/home/hermes` 中的状态会被保留（这些是绑定挂载）。
...
`preStart` 脚本会在 `${stateDir}/.gc-root` 创建一个指向当前 hermes 包的 GC 根。这可以防止 `nix-collect-garbage` 删除正在运行的二进制文件。如果 GC 根意外损坏，重启服务会重新创建它。
---
...
适用于仅包含 `plugin.yaml` + `__init__.py` 源码树的插件（例如 [hermes-lcm](https://github.com/stephenschoettler/hermes-lcm)）：
...
插件会在激活时被符号链接到 `$HERMES_HOME/plugins/` 目录。Hermes 会通过其常规目录扫描发现这些插件。从列表中移除插件并运行 `nixos-rebuild switch` 会删除对应的符号链接。

### 入口点插件（`extraPythonPackages`）

适用于通过 `[project.entry-points."hermes_agent.plugins"]` 注册的 pip 包插件（例如 [rtk-hermes](https://github.com/ogallotti/rtk-hermes)）：
...
该包的 `site-packages` 会被添加到 Hermes 包装器中的 PYTHONPATH。`importlib.metadata` 会在会话启动时自动发现入口点。

### 组合使用

依赖第三方 Python 包的目录插件需要同时使用这两个选项：
...
插件仍需在 `config.yaml` 中启用。可通过声明式设置添加：
...
:::note
构建时碰撞检查可防止插件包遮蔽 Hermes 核心依赖。如果插件提供了密封虚拟环境中已存在的包，`nixos-rebuild` 会因明确的错误而失败。
:::
...
# Shell 提供：
#   - Python 3.11 + uv（首次进入时依赖会安装到 .venv）
#   - Node.js 20、ripgrep、git、openssh、ffmpeg 在 PATH 中
#   - 标记文件优化：如果依赖未更改，重新进入几乎是瞬时的
...
| `package` | `package` | `hermes-agent` | 使用的 hermes-智能体 包 |
...
| `addToSystemPackages` | `bool` | `false` | 将 `hermes` 命令行工具添加到系统 PATH 并在系统范围内设置 `HERMES_HOME` |
...
| `mcpServers.<name>.command` | `null` 或 `str` | `null` | 服务器命令（标准输入/输出传输） |
...
| `container.extraVolumes` | `listOf str` | `[]` | 额外卷挂载（`主机:容器:模式`） |
...
```
/var/lib/hermes/                     # 状态目录（由 hermes:hermes 拥有，权限 0750）
├── .hermes/                         # HERMES_HOME
│   ├── config.yaml                  # 由 Nix 生成（每次重建时深度合并）
│   ├── .managed                     # 标记：CLI 配置修改被阻止
│   ├── .env                         # 从环境变量 + environmentFiles 合并
...
| `/usr`, `/usr/local`, `/tmp` | （可写层） | `rw` | `apt`/`pip`/`npm` 安装 —— 在重启之间保持，在重新创建时丢失 |
...
如果智能体启动但无法与 LLM 提供程序进行身份验证，请检查 `.env` 文件是否正确合并：
...
| 症状 | 原因 | 解决方法 |
|---|---|---|
| `无法保存配置：由 NixOS 管理` | CLI 保护激活 | 编辑 `configuration.nix` 并执行 `nixos-rebuild switch` |
...
| `找不到名为 "hermes-agent" 的容器`（Podman） | Podman 有根容器对常规用户不可见 | 为 podman 添加无密码 sudo（参见 [容器感知 CLI](#container-aware-cli) 部分） |
| `找不到用户 hermes` | 容器仍在启动中（入口点尚未创建用户） | 等待几秒钟后重试 —— CLI 会自动重试 |
